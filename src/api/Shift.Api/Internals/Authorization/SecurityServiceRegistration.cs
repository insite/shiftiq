...
public static partial class ServiceCollectionExtensions
{
    ...
    {
        services.AddSingleton<PermissionMatrixLoader>(f =>
        {
            var contextFactory = f.GetRequiredService<IDbContextFactory<TableDbContext>>();
            var routeSettings = f.GetRequiredService<RouteSettings>();
            var customRoutes = RouteScanner.Scan();

            return new PermissionMatrixLoader(contextFactory, routeSettings, customRoutes);
        });
