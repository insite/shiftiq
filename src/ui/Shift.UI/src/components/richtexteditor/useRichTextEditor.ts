...
export function useRichTextEditor(
    ...
    function handleTranslate() {
        runSave(async () => {
            ...
            // Needs to call the actual API endpoint

            await new Promise(resolve => setTimeout(resolve, 1000));

            let newMarkdown: MultiLanguageText | null = null;
            let newHtml: MultiLanguageText | null = null;

            if (currentValue?.markdown) {
                newMarkdown = {...currentValue?.markdown};
            ...
            if (currentValue?.html) {
                newHtml = {...currentValue?.html};
                for (const language of siteSetting.SupportedLanguages) {
                    if (language !== "en") {
                        newHtml[language] = newHtml.en + `(translated ${language})`;
                    }
                }
            }
