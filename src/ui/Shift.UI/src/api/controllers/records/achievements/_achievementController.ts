...
export const _achievementController = {
    async retrieve(achievementId: string) {
        return await fetchHelper.get<ApiAchievementModel | null>(`/api/records/achievements/${achievementId}`, null, true);
    },

    async search(query: ApiSearchAchievements, pageIndex: number, pageSize: number) {
        return await fetchHelper.getPagedRows<ApiAchievementMatch>(`/api/records/achievements/search`, query, pageIndex, pageSize, null, null);
    },
}
