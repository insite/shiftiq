...
export const _periodController = {
    async search(query: ApiSearchPeriods, pageIndex: number, pageSize: number) {
        return await fetchHelper.getPagedRows<ApiPeriodMatch>(`/api/records/periods/search`, query, pageIndex, pageSize, null, null);
    },

    async retrieve(periodId: string) {
        return await fetchHelper.get<ApiPeriodModel>(`/api/records/periods/${periodId}`);
    }
}
