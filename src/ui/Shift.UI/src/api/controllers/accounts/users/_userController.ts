...
export const _userController = {
    async search(query: ApiSearchUsers, pageIndex: number, pageSize: number, sortByColumn: string | null) {
        return await fetchHelper.getPagedRows<ApiUserMatch>(`/api/accounts/users/search`, query, pageIndex, pageSize, sortByColumn, null);
    },

    async retrieve(userId: string): Promise<ApiUserModel | null> {
        return await fetchHelper.get<ApiUserModel>(`/api/accounts/users/${userId}`, null, true);
    }
}
