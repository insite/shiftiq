import { fetchHelper } from "@/api/fetchHelper";
import { ApiPageContentModel } from "./ApiPageContentModel";
import { ApiPageContentModifyModel } from "./ApiPageContentModifyModel";

export const _pageContentController = {
    async retrieve(pageId: string): Promise<ApiPageContentModel | null> {
        return await fetchHelper.get<ApiPageContentModel | null>(`/api/sites/pages-contents/${pageId}`, null, true);
    },

    async modify(pageId: string, model: ApiPageContentModifyModel): Promise<Record<number, string> | null> {
        return await fetchHelper.put<Record<number, string> | null>(`/api/sites/pages-contents/${pageId}`, model);
    },
}
