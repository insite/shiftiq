...
export const _fileController = {
    async count(query: ApiSearchFiles) {
        const result = await fetchHelper.get<ApiCountResult>(`/api/assets/files/count`, fetchHelper.toParams(query));
        return result.Count;
    },

    async search(query: ApiSearchFiles, pageIndex: number, sortByColumn: string | null) {
        return await fetchHelper.getPagedRows<ApiFileMatch>(`/api/assets/files/search`, query, pageIndex, null, sortByColumn, null);
    },

    async download(query: ApiSearchFiles, format: ApiDownloadFormat, visibleColumns: string[]) {
        return await fetchHelper.download(`/api/assets/files/download`, query, format, visibleColumns);
    },
