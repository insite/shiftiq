...
export const _standardController = {
    async search(query: ApiSearchStandards, pageIndex: number, pageSize: number) {
        return await fetchHelper.getPagedRows<ApiStandardMatch>(`/api/standards/search`, query, pageIndex, pageSize, null, null);
    },

    async retrieve(standardId: string) {
        return await fetchHelper.get<ApiStandardModel>(`/api/standards/${standardId}`);
    }
}
