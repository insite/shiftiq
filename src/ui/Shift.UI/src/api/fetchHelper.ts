...
import { LOCAL_ENV_API_LATENCY } from "@/helpers/constants";
...
async function afterRequest<T>(response: Response, asBlob: boolean, returnNullOn404: boolean): Promise<T> {
    ...
    if (process.env.NODE_ENV === "development" && LOCAL_ENV_API_LATENCY > 0) {
        await new Promise(resolve => setTimeout(resolve, LOCAL_ENV_API_LATENCY));
    }
