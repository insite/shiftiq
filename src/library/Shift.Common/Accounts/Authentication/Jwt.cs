...
namespace Shift.Common
{
    public class Jwt : IJwt
    {
        ...
        {
            // Many external systems use Firebase to verify authentication tokens. Firebase considers a token invalid
            // when the value of the "iat" claim represents a time in the future. If there is a slight clock skew
            // between servers, then this can cause a token to be rejected even though it is valid on the server that
            // generated it. To mitigate this risk, we set the "iat" and "nbf" claims to a time one minute in the past.
        ...
        public bool Paging
        {
            get
            {
                var paging = GetClaimValue(ClaimName.Paging);

                if (Boolean.TryParse(paging, out bool result))
                    return result;
            ...
            set
            {
                SetValue(ClaimName.Paging, value.ToString());
            }
        }
