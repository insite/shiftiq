...
namespace Shift.Common
{
    public class Blowfish
    {
        ...
        private static readonly UInt32[,] _sBoxesInitial =
        {
            {
        ...
        private static readonly UInt32[] _pArrayInitial =
        {
        ...
        // Eric Young's test vectors for Blowfish.
        // http://www.schneier.com/code/vectors.txt

        /* plaintext bytes -- left halves */

        private static readonly UInt32[] _plainTextL =
        {
        ...
        private static readonly UInt32[] _plainTextR =
        {
        ...
        private static readonly Byte[][] _variableKey =
        {
        ...
        private static readonly UInt32[] _cipherTextL =
        {
        ...
        private static readonly UInt32[] _cipherTextR =
        {
        ...
        private UInt32[] _pArray;

        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1814:PreferJaggedArraysOverMultidimensional", MessageId = "Member")]
        private UInt32[,] _sBoxes;
        ...
        {
            InitializeBlowfish(GetEncryptionKey(key));
        }

        private static Byte[] GetEncryptionKey(String text)
        {
        ...
        {
            for (Int32 i = 0; i < _variableKey.Length; i++)
            {
                var blowfish = new Blowfish(_variableKey[i]);

                UInt32 xl = _plainTextL[i];
                UInt32 xr = _plainTextR[i];

                blowfish.Encipher(ref xl, ref xr);

                if (xl != _cipherTextL[i] || xr != _cipherTextR[i])
                    throw ApplicationError.Create("Test failed.");

                blowfish.Decipher(ref xl, ref xr);

                if (xl != _plainTextL[i] || xr != _plainTextR[i])
                    throw ApplicationError.Create("Test failed.");
            }
        }

        public String EncipherString(String input)
        {
            if (String.IsNullOrEmpty(input))
                return null;

            Byte[] data = _encoding.GetBytes(input);

            Int32 mod = data.Length%8;

            if (mod != 0)
            {
                var newData = new Byte[data.Length + 8 - mod];
        ...
        public String DecipherString(String input)
        {
            if (String.IsNullOrEmpty(input))
                return null;
            ...
            for (Int32 i = bytes.Length - 1; i >= 0; i--)
            {
                if (bytes[i] == 0)
                    len--;
                else
        ...
        {
            if (data.Length%8 != 0)
                throw ApplicationError.Create("Buffer has incorrect length.");

            for (Int32 i = 0, j = 4; i < data.Length; i += 8, j += 8)
            {
                var xl = (UInt32) ((data[i] << 24) | (data[i + 1] << 16) | (data[i + 2] << 8) | data[i + 3]);
                var xr = (UInt32) ((data[j] << 24) | (data[j + 1] << 16) | (data[j + 2] << 8) | data[j + 3]);
        ...
        {
            var d = (UInt16) (x & 0x00FF);
            x >>= 8;
            var c = (UInt16) (x & 0x00FF);
            x >>= 8;
            var b = (UInt16) (x & 0x00FF);
            x >>= 8;
            var a = (UInt16) (x & 0x00FF);
        ...
        private void Encipher(ref UInt32 xl, ref UInt32 xr)
        {
            ...
            for (Int32 i = 0; i < _pArray.Length - 2; i++)
            {
                Xl = Xl ^ _pArray[i];
                Xr = F(Xl) ^ Xr;
            ...
            Xr = Xr ^ _pArray[_pArray.Length - 2];
            Xl = Xl ^ _pArray[_pArray.Length - 1];

            xl = Xl;
            xr = Xr;
        }

        private void Decipher(ref UInt32 xl, ref UInt32 xr)
        {
            UInt32 temp;
            ...
            for (Int32 i = _pArray.Length - 1; i > 1; i--)
            {
                Xl = Xl ^ _pArray[i];
                Xr = F(Xl) ^ Xr;
        ...
        {
            _sBoxes = _sBoxesInitial.Clone() as UInt32[,];
            _pArray = _pArrayInitial.Clone() as UInt32[];

            if (_sBoxes == null || _pArray == null)
                throw ApplicationError.Create("Initialization Failed.");

            for (Int32 i = 0, j = 0; i < _pArray.Length; ++i)
            {
                ...
                _pArray[i] = _pArray[i] ^ data;
            }

            UInt32 datal = 0x00000000;
            UInt32 datar = 0x00000000;

            for (Int32 i = 0; i < _pArray.Length; i += 2)
            {
