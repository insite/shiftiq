...
{
    internal class EncryptionBuffer
    {
        ...
        public long BlockSize => _ic.InputBlockSize;
        public long KeySize => _keySize;

        public EncryptionBuffer(string keyStr, AutoSaltSizes saltSize, SymmetricEncryptionAlgorithm cryptoAlgorithm = SymmetricEncryptionAlgorithm.AES_256_CBC, PaddingMode paddingMode = PaddingMode.PKCS7)
        {
            var salt = EncryptionHelper.GenerateSalt((int)saltSize);
        ...
        public EncryptionBuffer(byte[] keyBytes, AutoSaltSizes saltSize, SymmetricEncryptionAlgorithm cryptoAlgorithm = SymmetricEncryptionAlgorithm.AES_256_CBC, PaddingMode paddingMode = PaddingMode.PKCS7)
        {
            var salt = EncryptionHelper.GenerateSalt((int)saltSize);
            _outBuffer.AddBytes(salt);
            if (saltSize == AutoSaltSizes.Salt32)
        ...
        public EncryptionBuffer(Rfc2898DeriveBytes key, SymmetricEncryptionAlgorithm cryptoAlgorithm = SymmetricEncryptionAlgorithm.AES_256_CBC, PaddingMode paddingMode = PaddingMode.PKCS7)
        {
            Initialize(key, cryptoAlgorithm, paddingMode);
        }

        public EncryptionBuffer(byte[] password, byte[] saltValueBytes, SymmetricEncryptionAlgorithm cryptoAlgorithm = SymmetricEncryptionAlgorithm.AES_256_CBC, PaddingMode paddingMode = PaddingMode.PKCS7)
        {
            Rfc2898DeriveBytes key = new Rfc2898DeriveBytes(password, saltValueBytes, 7);
            Initialize(key, cryptoAlgorithm, paddingMode);
        }

        public EncryptionBuffer(string password, byte[] saltValueBytes, SymmetricEncryptionAlgorithm cryptoAlgorithm = SymmetricEncryptionAlgorithm.AES_256_CBC, PaddingMode paddingMode = PaddingMode.PKCS7)
        {
            Rfc2898DeriveBytes key = new Rfc2898DeriveBytes(password, saltValueBytes, 7);
            Initialize(key, cryptoAlgorithm, paddingMode);
        }

        public EncryptionBuffer(string password, string salt, SymmetricEncryptionAlgorithm cryptoAlgorithm = SymmetricEncryptionAlgorithm.AES_256_CBC, PaddingMode paddingMode = PaddingMode.PKCS7)
        {
        ...
        private void Initialize(Rfc2898DeriveBytes key, SymmetricEncryptionAlgorithm cryptoAlgorithm, PaddingMode paddingMode)
        {
            _gotAllData = false;
