...
namespace Shift.Common
{
    ...
    [Serializable]
    public sealed class SmsGatewayIntegration : BaseIntegration<BaseEndpoint>
    {
        public SmsGatewayIntegration()
        {
            Endpoints = new BaseEndpoint[0];
        }

        public bool IsEqual(SmsGatewayIntegration other)
        {
            return IsShallowEqual(other)
                && BaseEndpoint.IsEqual(Endpoints, other.Endpoints);
        }

        public SmsGatewayIntegration Clone()
        {
            return new SmsGatewayIntegration
            {
                Enabled = Enabled,
                Endpoints = Endpoints.Select(x => x.Clone()).ToArray()
            };
    ...
    public class OrganizationIntegrations
    {
        ...
        public SmsGatewayIntegration SmsGateway { get; set; }
