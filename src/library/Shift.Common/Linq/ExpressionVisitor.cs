...
namespace Shift.Common.Linq
{
    ...
    internal abstract class ExpressionVisitor
    {
        ...
        protected virtual Expression VisitInvocation(InvocationExpression iv)
        {
            ...
            if (!Equals(args, iv.Arguments) || expr != iv.Expression) return Expression.Invoke(expr, args);
            return iv;
        }
        ...
        protected virtual Expression VisitMemberInit(MemberInitExpression init)
        {
            ...
            if (n != init.NewExpression || !Equals(bindings, init.Bindings)) return Expression.MemberInit(n, bindings);
            return init;
        }
        ...
        protected virtual MemberListBinding VisitMemberListBinding(MemberListBinding binding)
        {
            ...
            if (!Equals(initializers, binding.Initializers)) return Expression.ListBind(binding.Member, initializers);
            return binding;
        }
        ...
        protected virtual MemberMemberBinding VisitMemberMemberBinding(MemberMemberBinding binding)
        {
            ...
            if (!Equals(bindings, binding.Bindings)) return Expression.MemberBind(binding.Member, bindings);
            return binding;
        }
        ...
        protected virtual Expression VisitMethodCall(MethodCallExpression m)
        {
            ...
            if (obj != m.Object || !Equals(args, m.Arguments)) return Expression.Call(obj, m.Method, args);
            return m;
        }
        ...
        protected virtual Expression VisitNewArray(NewArrayExpression na)
        {
            ...
            if (!Equals(exprs, na.Expressions))
            {
                if (na.NodeType == ExpressionType.NewArrayInit)
                    return Expression.NewArrayInit(na.Type.GetElementType() ?? throw new InvalidOperationException(),
                        exprs);
                return Expression.NewArrayBounds(na.Type.GetElementType() ?? throw new InvalidOperationException(),
                    exprs);
            }
