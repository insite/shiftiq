...
namespace Shift.Common.Linq
{
	...
	class ExpressionExpander : ExpressionVisitor
	{
		...
		{
			if (m.Method.Name == "Invoke" && m.Method.DeclaringType == typeof (LinqExtensions2))
			{
			...
			if (m.Method.Name == "Compile" && m.Object is MemberExpression me)
			{
			...
			if (m.Method.Name == "AsExpandable" && m.Method.DeclaringType == typeof (LinqExtensions2))
		...
		protected override Expression VisitMemberAccess (MemberExpression m)
		{
			...
			if (m.Member.DeclaringType != null && m.Member.DeclaringType.Name.StartsWith ("<>"))
		...
		Expression TransformExpr(MemberExpression input)
		{
            ...
            if (input.Member is FieldInfo field && field.FieldType.IsSubclassOf(typeof(Expression)) 
             || input.Member is PropertyInfo prop && prop.PropertyType.IsSubclassOf(typeof(Expression)))
