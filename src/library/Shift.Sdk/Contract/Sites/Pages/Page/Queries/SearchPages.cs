using System;
using System.Collections.Generic;

using Shift.Common;

namespace Shift.Contract
{
    public class SearchPages : Query<IEnumerable<PageMatch>>, IPageCriteria
    {
        public Guid? ObjectId { get; set; }
        public Guid? OrganizationId { get; set; }
        public Guid? ParentPageId { get; set; }
        public Guid? SiteId { get; set; }

        public bool? IsAccessDenied { get; set; }
        public bool? IsHidden { get; set; }
        public bool? IsNewTab { get; set; }
        public bool? IsNullNavigateUrl { get; set; }

        public string AuthorName { get; set; }
        public string ContentControl { get; set; }
        public string ContentLabels { get; set; }
        public string Hook { get; set; }
        public string LastChangeType { get; set; }
        public string LastChangeUser { get; set; }
        public string NavigateUrl { get; set; }
        public string ObjectType { get; set; }
        public string PageIcon { get; set; }
        public string PageSlug { get; set; }
        public string PageType { get; set; }
        public string ParentPageSlug { get; set; }
        public string ParentPageType { get; set; }
        public string SiteDomain { get; set; }
        public string Title { get; set; }

        public int? Sequence { get; set; }

        public DateTimeOffset? AuthorDateSince { get; set; }
        public DateTimeOffset? AuthorDateBefore { get; set; }
        public DateTimeOffset? LastChangeTimeSince { get; set; }
        public DateTimeOffset? LastChangeTimeBefore { get; set; }
    }
}
