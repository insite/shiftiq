...
namespace InSite.Domain.Messages
{
    public class MessageAggregate : AggregateRoot
    {
        ...
        public void HandleMailoutCallback(Guid mailout, string mailingProvider, Guid callbackId, string recipient, 
            DateTime timestamp, string status, Dictionary<string, string> data)
        {
            if (status.IsEmpty() || mailingProvider.IsEmpty())
                return;
            ...
            Apply(new MailoutCallbackHandled(mailout, mailingProvider, callbackId, recipient, timestamp, status, data));
        }
