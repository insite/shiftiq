...
namespace InSite.Application.Organizations.Write
{
    public static class OrganizationCommandCreator
    {
        public static List<ICommand> Create(OrganizationState state, bool isNew)
        {
            ...
            result.Add(new ModifyOrganizationLogbookSettings(state.OrganizationIdentifier, state.Toolkits.Logbooks));
            result.Add(new ModifyOrganizationIdentification(state.OrganizationIdentifier, state.OrganizationCode, state.CompanyName, state.CustomerCode, state.CustomerNumber));
