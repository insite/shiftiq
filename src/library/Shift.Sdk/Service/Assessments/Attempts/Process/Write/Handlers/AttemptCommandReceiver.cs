...
namespace InSite.Application.Attempts.Write
{
    public class AttemptCommandReceiver
    {
        ...
        private readonly KeyedLock<Guid> _createLock = new KeyedLock<Guid>(TimeSpan.FromMinutes(1));
        ...
        {
            using (_createLock.Lock(c.Candidate))
            {
        ...
        {
            using (_createLock.Lock(c.LearnerUserIdentifier))
            {
