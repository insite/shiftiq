...
namespace InSite.Application.Contacts.Write
{
    public class PersonCommandCreator
    {
        ...
        {
            if (!StringHelper.EqualsCaseSensitive(_old?.AccountStatusName, _new.AccountStatusName, true))
                _result.Add(new ModifyPersonFieldText(_new.PersonIdentifier, PersonField.AccountStatusName, _new.AccountStatusName));
        ...
        private void AddOtherIntChanges()
        {
            ...
            if (_old?.MembershipStatusItemKey != _new.MembershipStatusItemKey)
                _result.Add(new ModifyPersonFieldInt(_new.PersonIdentifier, PersonField.MembershipStatusItemKey, _new.MembershipStatusItemKey));
