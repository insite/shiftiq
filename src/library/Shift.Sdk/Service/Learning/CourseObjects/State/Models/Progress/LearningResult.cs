...
namespace InSite.Domain.CourseObjects
{
    ...
    public class LearningResult
    {
        ...
        public string Icon
        {
            ...
            {
                if (IsCompleted)
                    return "fas fa-fw fa-check text-success";

                if (IsLocked)
                    return "fas fa-fw fa-lock-alt text-danger";

                if (Status == "Completed" && (Grade.IsNotEmpty() || Score.HasValue) && IsFailed || Status == "Incomplete")
                    return "fas fa-fw fa-times text-danger";

                if (IsStarted)
                    return "fas fa-fw fa-hourglass-start text-info";

                return "fas fa-fw fa-circle text-primary";
            }
        }
