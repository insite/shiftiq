...
namespace InSite.Application.Organizations.Write
{
    public class ModifyOrganizationIntegrationSettings : Command, IHasRun
    {
        ...
        bool IHasRun.Run(OrganizationAggregate aggregate)
        {
            ...
            var isSmsGatewaySame = state.Integrations.SmsGateway.IsEqual(Integrations.SmsGateway);
            ...
            if (isBamboraSame && isBCMailSame && isRecaptchaSame && isSmsGatewaySame && isPrometricSame && isScormCloudSame)
                return true;
