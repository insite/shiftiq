...
namespace InSite.Application.Records.Read
{
    ...
    public class GradebookChangeProcessor
    {
        ...
        private void ViewUserProgramTaskEnrollment(QProgress progress, TCourse course)
        {
            _programStore.TaskViewed(progress.UserIdentifier, course.OrganizationIdentifier, course.CourseIdentifier);
        }

        private void CompleteUserProgramTaskEnrollment(QProgress progress, TCourse course)
        {
        ...
        private void SendProgramCompletionNotificationMessage(QProgress progress, TCourse course)
        {
        ...
        private void HandleCompletedCourse(ProgressCompleted2 e)
        {
            ...
            var courseUser = _courses.GetCourseUser(course.CourseIdentifier, progress.User);
            if (courseUser == null
                || courseUser.CourseCompleted.HasValue
                || course.CourseMessageCompletedToLearner == null && course.CourseMessageCompletedToAdministrator == null
                )
            {
                return;
            }
            ...
            var user = _contacts.GetUser(courseUser.UserIdentifier);
            var content = _contents.GetBlock(course.CourseIdentifier);
            var timeZone = TimeZoneInfo.FindSystemTimeZoneById(user.UserTimeZone);
            var courseStarted = TimeZones.FormatDateOnly(courseUser.CourseStarted, timeZone);
            ...
            if (course.CourseMessageCompletedToAdministrator.HasValue)
            {
                notification.MessageIdentifier = course.CourseMessageCompletedToAdministrator;
                _mailer.Send(notification, null);
            }

            if (course.CourseMessageCompletedToLearner.HasValue)
            {
                notification.MessageIdentifier = course.CourseMessageCompletedToLearner;
                _mailer.Send(notification, notification.LearnerIdentifier);
                _courseStore.IncreaseMessageCompletedSentCount(courseUser.CourseIdentifier, courseUser.EnrollmentIdentifier);
            }
        }
