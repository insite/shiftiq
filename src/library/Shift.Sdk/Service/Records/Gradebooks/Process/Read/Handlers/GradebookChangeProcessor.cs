...
namespace InSite.Application.Records.Read
{
    ...
    public class GradebookChangeProcessor
    {
        ...
        private void HandleItemAchievement(IChange change, TriggerCauseChange cause, decimal? percent)
        {
            ...
            var whenPassCommand = item.AchievementWhenPassCommand.ToEnumNullable<TriggerEffectCommand>();
            var whenFailCommand = item.AchievementWhenFailCommand.ToEnumNullable<TriggerEffectCommand>();
            var elseCommand = item.AchievementElseCommand.ToEnumNullable<TriggerEffectCommand>();

            if (!whenPassCommand.HasValue || !whenFailCommand.HasValue || !elseCommand.HasValue)
                return;
...

            {
                ...
                var effectCommand = percent != null
                    ? percent >= item.GradeItemPassPercent
                        ? whenPassCommand.Value
                        : whenFailCommand.Value
                    : elseCommand.Value;
                ...
                var commands = BuildCommands(effectCommand, gradebook.OrganizationIdentifier, item.AchievementIdentifier.Value, progress.UserIdentifier, item.AchievementFixedDate, reason, percent);
