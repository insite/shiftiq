...
namespace InSite.Domain.Records
{
    ...
    public class JournalSetupState : AggregateState
    {
        ...
        public bool ContainsGroup(Guid group, JournalSetupUserRole role)
            => Groups.Any(x => x.Identifier == group && x.Role == role);
        ...
        {
            Groups.Add(new JournalSetupGroup { Identifier = e.Group, Role = e.Role });
        }

        public void When(JournalSetupGroupRemoved e)
        {
            var index = Groups.FindIndex(x => x.Identifier == e.Group && x.Role == e.Role);
            if (index < 0)
