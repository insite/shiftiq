...
namespace InSite.Application.Records.Write
{
    public class JournalSetupCommandReceiver
    {
        ...
        public void Handle(CreateJournalSetupGroup c)
        {
            _repository.LockAndRun<JournalSetupAggregate>(c.AggregateIdentifier, (aggregate) =>
            {
                if (aggregate.Data.ContainsGroup(c.Group, c.Role))
                    throw new JournalSetupException($"Group '{c.Group}' with role '{c.Role}' already exists in journal setup {aggregate.AggregateIdentifier}");

                aggregate.CreateGroup(c.Group, c.Role);
                Commit(aggregate, c);
            });
        }
        ...
        public void Handle(RemoveJournalSetupGroup c)
        {
            _repository.LockAndRun<JournalSetupAggregate>(c.AggregateIdentifier, (aggregate) =>
            {
                if (aggregate.Data.ContainsGroup(c.Group, c.Role))
                {
                    aggregate.RemoveGroup(c.Group, c.Role);
                    Commit(aggregate, c);
                }
            });
        }
