...
public class MembershipReader : IEntityReader
{
    ...
    private IQueryable<MembershipEntity> BuildQueryable(TableDbContext db, IMembershipCriteria criteria, Guid? currentUserId)
    {
        ...
        if (criteria.MembershipEffectiveSince.HasValue)
            query = query.Where(x => x.MembershipEffective >= criteria.MembershipEffectiveSince);

        if (criteria.MembershipEffectiveBefore.HasValue)
            query = query.Where(x => x.MembershipEffective < criteria.MembershipEffectiveBefore);

        if (criteria.MembershipExpirySince.HasValue)
            query = query.Where(x => x.MembershipExpiry >= criteria.MembershipExpirySince);

        if (criteria.MembershipExpiryBefore.HasValue)
            query = query.Where(x => x.MembershipExpiry < criteria.MembershipExpiryBefore);
    ...
    {
        var adapter = new MembershipAdapter();
