...
public class GroupReader : IEntityReader
{
    ...
    private IQueryable<GroupEntity> BuildQueryable(TableDbContext db, IGroupCriteria criteria)
    {
        ...
        if (criteria.GroupCreatedBefore.HasValue)
            query = query.Where(x => x.GroupCreated < criteria.GroupCreatedBefore);

        if (criteria.GroupExpirySince.HasValue)
            query = query.Where(x => x.GroupExpiry >= criteria.GroupExpirySince);

        if (criteria.GroupExpiryBefore.HasValue)
            query = query.Where(x => x.GroupExpiry < criteria.GroupExpiryBefore);
