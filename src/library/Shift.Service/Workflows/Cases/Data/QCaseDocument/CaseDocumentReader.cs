...
public class CaseDocumentReader : IEntityReader
{
    ...
    private IQueryable<CaseDocumentEntity> BuildQueryable(TableDbContext db, ICaseDocumentCriteria criteria)
    {
        ...
        if (criteria.AttachmentPostedSince.HasValue)
            query = query.Where(x => x.AttachmentPosted >= criteria.AttachmentPostedSince);

        if (criteria.AttachmentPostedBefore.HasValue)
            query = query.Where(x => x.AttachmentPosted < criteria.AttachmentPostedBefore);
