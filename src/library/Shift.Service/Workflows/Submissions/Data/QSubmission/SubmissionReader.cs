...
public class SubmissionReader : IEntityReader
{
    ...
    private IQueryable<SubmissionEntity> BuildQueryable(TableDbContext db, ISubmissionCriteria criteria)
    {
        ...
        if (criteria.ResponseSessionCompletedSince.HasValue)
            query = query.Where(x => x.ResponseSessionCompleted >= criteria.ResponseSessionCompletedSince);

        if (criteria.ResponseSessionCompletedBefore.HasValue)
            query = query.Where(x => x.ResponseSessionCompleted < criteria.ResponseSessionCompletedBefore);

        if (criteria.ResponseSessionCreatedSince.HasValue)
            query = query.Where(x => x.ResponseSessionCreated >= criteria.ResponseSessionCreatedSince);

        if (criteria.ResponseSessionCreatedBefore.HasValue)
            query = query.Where(x => x.ResponseSessionCreated < criteria.ResponseSessionCreatedBefore);

        if (criteria.ResponseSessionStartedSince.HasValue)
            query = query.Where(x => x.ResponseSessionStarted >= criteria.ResponseSessionStartedSince);

        if (criteria.ResponseSessionStartedBefore.HasValue)
            query = query.Where(x => x.ResponseSessionStarted < criteria.ResponseSessionStartedBefore);
