...
public class FileActivityReader : IEntityReader
{
    ...
    private IQueryable<FileActivityEntity> BuildQueryable(TableDbContext db, IFileActivityCriteria criteria)
    {
        ...
        if (criteria.ActivityTimeSince.HasValue)
            query = query.Where(x => x.ActivityTime >= criteria.ActivityTimeSince);

        if (criteria.ActivityTimeBefore.HasValue)
            query = query.Where(x => x.ActivityTime < criteria.ActivityTimeBefore);
