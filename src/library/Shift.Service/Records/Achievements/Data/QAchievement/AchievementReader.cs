...
public class AchievementReader : IEntityReader
{
    ...
    private IQueryable<AchievementEntity> BuildQueryable(TableDbContext db, IAchievementCriteria criteria)
    {
        ...
        if (criteria.ExpirationFixedDateSince.HasValue)
            query = query.Where(x => x.ExpirationFixedDate >= criteria.ExpirationFixedDateSince);

        if (criteria.ExpirationFixedDateBefore.HasValue)
            query = query.Where(x => x.ExpirationFixedDate < criteria.ExpirationFixedDateBefore);
