...
namespace Shift.Service
{
    ...
    public class PermissionMatrixLoader : PermissionMatrixLoaderBase
    {
        ...
        private readonly IEnumerable<CustomPermissionRoute> _customRoutes;

        public PermissionMatrixLoader(IDbContextFactory<TableDbContext> contextFactory, RouteSettings routeSettings, IEnumerable<CustomPermissionRoute> customRoutes)
        {
        ...
        protected override IReadOnlyList<RouteEndpoint> GetRoutes()
        {
            ...
            var routes = db.RouteEndpoint.ToList();
            var routeSet = routes.Select(x => x.RouteUrl.ToLower()).ToHashSet();

            foreach (var customRoute in _customRoutes)
            {
                if (routeSet.Contains(customRoute.Url.ToLower()))
                    continue;

                routes.Add(new RouteEndpoint
                {
                    RouteId = UniqueIdentifier.Create(),
                    RouteType = "API",
                    RouteUrl = customRoute.Url,
                    RouteDepth = 0,
                    ParentRouteId = null,
                    ResourcePath = customRoute.Resource
                });
            }
