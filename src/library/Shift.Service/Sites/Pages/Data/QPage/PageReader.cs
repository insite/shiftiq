...
public class PageReader : IEntityReader
{
    ...
    private IQueryable<PageEntity> BuildQueryable(TableDbContext db, IPageCriteria criteria)
    {
        ...
        if (criteria.AuthorDateSince.HasValue)
            query = query.Where(x => x.AuthorDate >= criteria.AuthorDateSince);

        if (criteria.AuthorDateBefore.HasValue)
            query = query.Where(x => x.AuthorDate < criteria.AuthorDateBefore);
