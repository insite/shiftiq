...
public class TUserSessionWriter : IEntityWriter
{
    private readonly IDbContextFactory<TableDbContext> _context;

    public TUserSessionWriter(IDbContextFactory<TableDbContext> context)
    {
        _context = context;
    }

    public async Task<bool> CreateAsync(TUserSessionEntity entity, CancellationToken cancellation = default)
    {
    ...
    public async Task<bool> ModifyAsync(TUserSessionEntity entity, CancellationToken cancellation = default)
    {
        using var db = _context.CreateDbContext();
