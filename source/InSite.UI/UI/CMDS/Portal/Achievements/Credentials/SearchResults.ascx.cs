...
namespace InSite.UI.CMDS.Portal.Achievements.Credentials
{
    public partial class SearchResults : UserControl
    {
        ...
        private void AchievementCategoryRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            var grid = (Grid)e.Item.FindControl("Grid");
            grid.RowDataBound += Grid_RowDataBound;
            grid.DataSource = group.AchievementItems;
            grid.DataBind();
        }
        ...
        private void Grid_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType != DataControlRowType.DataRow)
                return;
            BindDataItem(e.Row, (AchievementItem)e.Row.DataItem);
            BindAttachments(e.Row, (AchievementItem)e.Row.DataItem);
        }
        ...
        public static void BindDataItem(Control control, AchievementItem item)
        {
            ...
            var lifetimeMonths = item.LifetimeMonths;

            var timeSensitiveIcon = (Icon)control.FindControl("TimeSensitiveIcon");
            TimeSensitivityHelper.SetTimeSensitiveIcon(lifetimeMonths.HasValue, ValidForUnits.Months, lifetimeMonths,
                timeSensitiveIcon);
            ...
            if (t != null)
            {
                if (item.IsSkillsPassport == true)
                {
                    t.Text =
                        title
                        + (!isRequired ? " <i class='far fa-dot-circle' title='Optional (Not Required)'></i>" : "")
                        ;
                }
                else
                {
                    ...
                    t.Text =
                        $@"<a href='{url}'>{title}</a>"
                        + (!isRequired ? "<i class='ms-2 fas fa-triangle-exclamation text-info' title='Not Required'></i>" : "")
                        + (!isPlanned ? "<i class='ms-2 fas fa-do-not-enter text-danger' title='Not In Training Plan'></i>" : "")
                        ;
                }
            }
        }
