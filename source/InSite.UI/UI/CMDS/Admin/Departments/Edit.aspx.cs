...
namespace InSite.Cmds.Admin.Departments.Forms
{
    public partial class Edit : AdminBasePage, ICmdsUserControl, IHasParentLinkParameters
    {
        ...
        private void Open()
        {
            ...
            var profileCount = ProfileEditor.LoadProfiles(null, DepartmentIdentifier);
            ProfileTab.SetTitle("Profiles", profileCount);

            AchievementEditor.SetEditable(CanEdit, CanEdit);
            var achievementCount = AchievementEditor.LoadAchievements();
            AchievementTab.SetTitle("Achievements", achievementCount);
            ...
            var competencyCount = DepartmentProfileCompetencyRepository2.SelectCompetencyLevels(department.DepartmentIdentifier);
