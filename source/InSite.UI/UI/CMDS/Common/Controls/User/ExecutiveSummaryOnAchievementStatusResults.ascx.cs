...
namespace InSite.Custom.CMDS.Admin.Reports.Controls
{
    public partial class ExecutiveSummaryOnAchievementStatusResults : SearchResultsGridViewController<ExecutiveSummaryOnAchievementStatusFilter>
    {
        ...
        {
            if (score == null)
                return "NA <span class='text-body-secondary'><i class='fas fa-fw fa-circle'></i></span>";

            score = score / 100.0m;

            if (score >= 1.0m)
                return $"{score:p0} <span class='text-success'><i class='fas fa-fw fa-flag-checkered'></i></span>";

            if (score >= 0.5m)
                return $"{score:p0} <span class='text-warning'><i class='far fa-fw fa-flag'></i></span>";

            return $"{score:p0} <span class='text-danger'><i class='far fa-fw fa-flag'></i></span>";
        }
    }
}