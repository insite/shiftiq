...
namespace InSite.Cmds.User.Competencies.Controls
{
    public partial class CompetencyAchievementAndDownloadViewer : UserControl
    {
        ...
        private void AchievementsRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                var course = CourseSearch.BindCourseFirst(x => x.CourseIdentifier, x => x.Gradebook.AchievementIdentifier == achievement.AchievementIdentifier);

                if (course != null && TGroupPermissionSearch.IsAccessAllowed(course, CurrentSessionState.Identity))
                {
                    var link = Custom.CMDS.Portal.Courses.CmdsCourseLink.GetCourseLink(course);
