...
<insite:Grid runat="server" ID="Grid">
    <Columns>
        ...
        <asp:TemplateField HeaderText="Column" SortExpression="ColumnName">
            <ItemTemplate>
                <a href="/ui/admin/schemas/columns/outline?<%# string.Format("schemaName={0}&tableName={1}&columnName={2}", Eval("SchemaName"), Eval("TableName"), Eval("ColumnName")) %>"><%# Eval("ColumnName") %></a>
            </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField ItemStyle-Width="20px">
            <ItemTemplate>
                <insite:IconLink runat="server" Name="database" ToolTip="View"
                    NavigateUrl='<%# string.Format("/ui/admin/schemas/columns/outline?schemaName={0}&tableName={1}&columnName={2}", Eval("SchemaName"), Eval("TableName"), Eval("ColumnName")) %>' />
            </ItemTemplate>
        </asp:TemplateField>
