...
<div class="d-inline-block">
    ...
    <insite:Grid runat="server" ID="Grid" DataKeyNames="LearnerIdentifier" CssClass="table table-bordered table-striped table-responsive">
        <Columns>
            ...
            <asp:BoundField HeaderText="Consent" DataField="LearnerConsent" />
            ...
            <asp:BoundField HeaderText="Referrer Name" DataField="ReferrerName" />
            <asp:BoundField HeaderText="Referrer Other" DataField="ReferrerOther" />

            <asp:BoundField HeaderText="Citizenship" DataField="LearnerCitizenship" />
            <asp:BoundField HeaderText="Immigration Number" DataField="ImmigrationNumber" />
            <asp:TemplateField HeaderText="Immigration Arrival" ItemStyle-Wrap="false">
                <ItemTemplate>
                    <%# Eval("ImmigrationArrival", "{0:MMM d, yyyy}") %>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Immigration Status">
                <HeaderTemplate>
                    <span title="As at time of account creation.">Immigration Status</span>
                </HeaderTemplate>
                <ItemTemplate>
                    <%# Eval("ImmigrationStatus") %>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:BoundField HeaderText="Immigration Destination" DataField="ImmigrationDestination" />

            <asp:TemplateField HeaderText="Achievement Granted" ItemStyle-Wrap="false">
                <ItemTemplate>
                    <%# Eval("CertificateGranted", "{0:MMM d, yyyy}") %>
                </ItemTemplate>
            </asp:TemplateField>
