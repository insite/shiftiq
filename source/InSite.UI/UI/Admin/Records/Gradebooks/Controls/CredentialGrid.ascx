<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CredentialGrid.ascx.cs" Inherits="InSite.Admin.Records.Gradebooks.Controls.CredentialGrid" %>

<div class="row mb-1">
    <div class="col-lg-4">
        <insite:ComboBox runat="server" ID="AchievementType" Width="300">
            <Items>
                <insite:ComboBoxOption Value="Gradebook" Text="Gradebook Achievements Only" Selected="true" />
    ...
    <div class="col-lg-8 overflow-hidden">
        <asp:Repeater runat="server" ID="AchievementRepeater">
            ...
            <ItemTemplate>
                <insite:Button runat="server" ButtonStyle="Default" CssClass='<%# (AchievementIdentifier == (Guid)Eval("AchievementIdentifier") ? "active" : "") %>'
                    CommandName="Filter"
                    CommandArgument='<%# Eval("AchievementIdentifier") %>'
                    ToolTip="Filter"
                    Icon="far fa-cube"
                    Text='<%# Eval("AchievementTitle") + " (" + Eval("Count", "{0:n0}") + ")" %>' />
            </ItemTemplate>
        </asp:Repeater>
    </div>
</div>
