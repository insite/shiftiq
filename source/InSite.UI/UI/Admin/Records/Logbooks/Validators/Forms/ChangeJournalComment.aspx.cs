...
namespace InSite.UI.Admin.Records.Validators.Forms
{
    public partial class ChangeJournalComment : AdminBasePage, IOverrideWebRouteParent, IHasParentLinkParameters
    {
        ...
        private void LoadData()
        {
            ...
            if (!ServiceLocator.JournalSearch.ExistsJournalSetupUser(journal.JournalSetupIdentifier, User.UserIdentifier, JournalSetupUserRole.Validator))
                RedirectToSearch();
            ...
            if (!Detail.SetInputValues(journal.JournalSetupIdentifier, journal.UserIdentifier, comment))
            {
                RedirectToSearch();
                return;
            }
        ...
        private string GetReturnUrlInternal() => GetReturnUrl()
            .IfNullOrEmpty($"/ui/admin/records/logbooks/validators/outline-journal?journalsetup={JournalSetupIdentifier}&user={UserIdentifier}&panel=comments");
