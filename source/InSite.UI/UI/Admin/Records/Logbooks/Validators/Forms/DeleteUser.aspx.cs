    ...
    public partial class DeleteUser : AdminBasePage, IOverrideWebRouteParent, IHasParentLinkParameters
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                PageHelper.AutoBindHeader(this, null, LogbookHeaderHelper.GetLogbookHeader(user.JournalSetup, User.TimeZone));

                LogbookName.Text = $"<a href=\"/ui/admin/records/logbooks/outline?journalsetup={user.JournalSetupIdentifier}\">{user.JournalSetup.JournalSetupName}</a>";
                var content = ServiceLocator.ContentSearch.GetBlock(user.JournalSetupIdentifier, MultilingualString.DefaultLanguage);
        ...
        private void LoadData()
        {
            ...
            if (entryCount > 0)
            {
                if (UserIdentifier == User.Identifier)
