...
namespace InSite.UI.Admin.Contacts.People.Controls
{
    public partial class EditTabSignIn : BaseUserControl
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            IsJobsApproved.CheckedChanged += (sender, args) =>
            {
                if (IsJobsApproved.Checked)
                    UserAccountApproved.Value = TimeZones.ConvertFromUtc(DateTimeOffset.UtcNow, User.TimeZone);
                else
                    UserAccountApproved.Value = null;
            };
        }
        ...
        public void SetInputValues(QPerson person)
        {
            ...
            JobsAccessField.Visible = IsJobsApproved.Visible = Organization.IsIndustrySpecialist();
            IsJobsApproved.Checked = person.JobsApproved.HasValue;
            IsJobsApprovedDateTime.Text = person.JobsApproved.HasValue ? TimeZones.Format(person.JobsApproved.Value, Identity.User.TimeZone, true, false) : "";
            IsJobsApprovedDateTime.Visible = person.JobsApproved.HasValue;
        ...
        public void BindRoles()
        {
            ...
            foreach (var role in roles)
                if (role.Selected)
                    RoleCheckList.Items.FindByValue(role.GroupIdentifier.ToString()).Selected = true;
        }

        public void AppendRoles(Guid organizationId, List<UserRoleItem> list)
        {
