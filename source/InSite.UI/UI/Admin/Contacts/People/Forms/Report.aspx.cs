...
namespace InSite.Admin.Contacts.People.Forms
{
    public partial class Report : AdminBasePage, IHasParentLinkParameters
    {
        ...
        private void LoadData()
        {
            ...
            if (membershipStatus.IsNotEmpty())
            {
                ...
                if (person.MemberStartDate.HasValue)
                    MembershipStatus.Text += $" from {person.MemberStartDate:MMM d, yyyy}";
                if (person.MemberEndDate.HasValue)
                    MembershipStatus.Text += $" till {person.MemberEndDate:MMM d, yyyy}";
            }
            else
                MembershipStatus.Text = "None";
            ...
            LoadBCPVPAData(person.User, person, employer, employerShippingAddress, membershipStatus);
        ...
        private void LoadBCPVPAData(QUser user, QPerson person, QGroup employer, QGroupAddress employerShippingAddress, string membershipStatus)
        {
            var isBCPVPA = Organization.OrganizationIdentifier == Shift.Constant.OrganizationIdentifiers.BCPVPA;

            BCPVPAMembershipSection.Visible = isBCPVPA;
            PrintButton.Visible = isBCPVPA;
            ...
            if (isBCPVPA)
            {
                ...
                if (employer != null)
                {
                    ...
                    BCPVPASchoolNumber.Text = employer.GroupCode.HasValue() ? "#" + employer.GroupCode : "";
                    BCPVPASchoolName.Text = employer.GroupName.HasValue() ? employer.GroupName : "None";
                    if (employerShippingAddress != null)
                        BCPVPASchoolAddress.Text = ClassVenueAddressInfo.GetAddress(employerShippingAddress);
                }
                else
                {
                ...
                BCPVPASchoolDisctrict.Text = employer?.Parent?.GroupName;
                ...
                if (membershipStatus.IsNotEmpty())
                {
                    ...
                    if (person.MemberStartDate.HasValue)
                        BCPVPAMembershipStatus.Text += $" from {person.MemberStartDate:MMM d, yyyy}";
                    if (person.MemberEndDate.HasValue)
                        BCPVPAMembershipStatus.Text += $" till {person.MemberEndDate:MMM d, yyyy}";
                }
                else
                    BCPVPAMembershipStatus.Text = "None";
        ...
        private void BindSurveys(QUser user)
        {
            ...
            SurveySectionTitle.Text = $"Surveys ({SurveyDataSource.Length:n0})";
        }
