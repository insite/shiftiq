...
namespace InSite.Admin.Courses.Outlines.Controls
{
    public partial class ActivitySetup : BaseUserControl
    {
        ...
        private void ActivitySaveButton_Click(object sender, EventArgs e)
        {
            ...
            {
                if (ex.Message.StartsWith("Cannot insert duplicate key row in object 'courses.TActivity' with unique index 'IX_TActivity_GradeItem'"))
        ...
        private void SaveChangesToGradeItem(TCourse course, QActivity activity)
        {
        ...
        private void SaveChangesToAchievement(TCourse course, QActivity activity)
        {
        ...
        private void SaveChangesToCompetencies(TCourse course, QActivity activity)
        {
            if (!course.FrameworkIdentifier.HasValue)
                return;
            ...
            {
                var framework = InSite.Admin.Courses.Activities.Controls.CompetenciesSelector.LoadCompetencies(activity.ActivityIdentifier, course.FrameworkIdentifier.Value)
                    .FirstOrDefault(x => x.StandardIdentifier == course.FrameworkIdentifier.Value);
        ...
        public void BindModelToControls(Course course, TActivity activity)
        {
