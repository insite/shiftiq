...
namespace InSite.Admin.Courses.Courses
{
    public class Course2Deserializer
    {
        ...
        private Guid? GetSurvey(string name)
        {
            ...
            {
                _result.Warnings.Add($"Survey '{name}' is used more than once in this course.");
                return null;
            }
            ...
            if (identifier == null)
            {
                _result.Warnings.Add($"Survey '{name}' is not found.");
            }
            else if (CourseSearch.ActivityExists(x => x.SurveyFormIdentifier == identifier))
            {
                _result.Warnings.Add($"Survey '{name}' is already used by another course.");
                identifier = null;
            }
