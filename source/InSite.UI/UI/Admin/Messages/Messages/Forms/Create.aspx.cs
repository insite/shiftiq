...
namespace InSite.Admin.Messages.Messages.Forms
{
    public partial class Create : AdminBasePage, IOverrideWebRouteParent, IHasParentLinkParameters
    {
        ...
        private void SetupMessageType(out string messageType)
        {
            ...
            if (Guid.TryParse(Request.QueryString["survey"], out var surveyFormId))
            {
        ...
        private void SaveOne()
        {
            ...
            // Check to see if there is an existing Survey Invitation message assigned to the same survey form. If so 
            ...
            if (model.MessageType == MessageTypeName.Invitation && model.SurveyFormIdentifier.HasValue)
            {
                ...
                if (ServiceLocator.MessageSearch.CountMessages(filter) > 0)
                {
                    ScreenStatus.AddMessage(
                        AlertType.Error,
                        "A survey invitation message already exists for the survey form you have selected. Please select a different survey form, or use the existing message to send invitations for this survey.");

                    return;
                }

                modelContent.Body.Text.Default = @"Please complete this survey by clicking the **Begin Survey** button.
