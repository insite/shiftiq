...
namespace InSite.Admin.Messages.Mailouts.Forms
{
    public partial class Schedule : AdminBasePage, IHasParentLinkParameters
    {
        ...
        private RecipientInfo[] GetRecipients(VMessage message, Guid? recipientId)
        {
            ...
            // If this is a survey invitation then existing respondents should be excluded automatically from
            // the recipient list.
...
These subscribers will not get this message because their email has been disabled:
<ul class='fst-italic'>");
...
<ul class='fst-italic'>");
        ...
        private (string[] disabled, string[] marketingDisabled) GetUnsubscribers(IReadOnlyCollection<RecipientInfo> recipients)
        {
            ...
            var marketingDisabled = new List<string>();

            foreach (var recipient in recipients)
            {
                if (!recipient.UserEmailEnabled
                    && (!recipient.UserEmailAlternateEnabled || string.IsNullOrEmpty(recipient.UserEmailAlternate))
                    )
                {
                    disabled.Add(recipient.UserEmail);
                }
                else if (!recipient.UserEmailAlternateEnabled && !string.IsNullOrEmpty(recipient.UserEmailAlternate))
                    disabled.Add(recipient.UserEmailAlternate);
                else if (IsNewsletter && !recipient.UserMarketingEmailEnabled)
                    marketingDisabled.Add(recipient.UserEmail);
            }

            return (disabled.ToArray(), marketingDisabled.ToArray());
        }
        ...
        {
            // Allow a special-case pattern for survey links. The URL here is resolved in the Send method.
