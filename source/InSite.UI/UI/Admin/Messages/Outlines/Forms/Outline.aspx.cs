    ...
    public partial class Outline : AdminBasePage
    {
        ...
        private void SetupContactSection()
        {
            if (Entity.SurveyFormIdentifier.HasValue)
            {
                ...
                RespondentsCount.Text = "survey invitation recipient".ToQuantity(junction, "n0") + " submitted a response";
                DeleteRespondents.OnClientClick = $"return confirm('Are you sure you want to remove from this survey invitation the {"respondent".ToQuantity(junction)} who answered the survey?');";
            }
        ...
        private void SendRespondentsCsv()
        {
            ...
            {
                ScreenStatus.AddMessage(AlertType.Error, "There are no survey invitation recipients who have submitted a response.");
                SetupContactSection();
                return;
            }
