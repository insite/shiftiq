...
<insite:Grid runat="server" ID="Grid" DataKeyNames="OrganizationIdentifier" Translation="Header">
    <Columns>
        ...
        <asp:TemplateField HeaderText="Action" HeaderStyle-Width="40px" ItemStyle-Wrap="false" ItemStyle-CssClass="text-end" HeaderStyle-CssClass="text-end">
            <ItemTemplate>
                <a runat="server" href='<%# Eval("ReportIdentifier", "/ui/admin/reports/edit?id={0}") %>'><i class="fa-fw far fa-pencil"></i></a>
                <a runat="server" href='<%# Eval("ReportIdentifier", "/ui/admin/reports/delete?id={0}") %>' visible='<%# Eval("CanDelete") %>'><i class="fa-fw far fa-trash-alt"></i></a>
                <a runat="server" href='<%# Eval("ReportIdentifier", "/ui/admin/reports/build?id={0}&execute=true") %>' visible='<%# Eval("CanExecute") %>'><i class="fa-fw far fa-bolt"></i></a>
                <a runat="server" href='<%# Eval("ReportIdentifier", "/ui/admin/reports/build?id={0}") %>' visible='<%# Eval("CanConfigure") %>'><i class="fa-fw far fa-cog"></i></a>
            </ItemTemplate>
        </asp:TemplateField>
