...
namespace InSite.Admin.Assets.Contents.Controls.ContentEditor
{
    public partial class SectionTiles : SectionBase
    {
        ...
        protected override void CreateChildControls()
        {
            if (Container.Controls.Count == 0 && TileItems.Count > 0)
            {
                for (var i = 0; i < TileItems.Count; i++)
                    AddTile(TileItems[i].Title, TileItems[i].Path, out _);
            }
        ...
        {
            if (options is AssetContentSection.TileList tileList)
            {
                TileSize = tileList.Size;

                foreach (var tile in tileList.Options)
                {
                    AddTile(tile.Id, tile.Title, tile.ControlPath, out var tileCtrl);
        ...
        public MultilingualString GetValue(string id, Enum id2)
        {
            if (!string.IsNullOrEmpty(id) && Identifiers.TryGetValue(id, out var index))
                return _controls[index].GetValue(id2.GetName());
            else
                throw ApplicationError.Create("Section not found: " + id);
        }
        ...
        private void AddTile(string id, string title, string tilePath, out SectionBase tile)
        {
            if (Identifiers.ContainsKey(id))
                throw ApplicationError.Create("Invalid tile ID: " + id);

            AddTile(title, tilePath, out tile);
        ...
        private void AddTile(string title, string tilePath, out SectionBase tile)
        {
            ...
            if (!string.IsNullOrEmpty(title))
                Container.Controls.Add(new LiteralControl($"<h3>{title}</h3>"));

            Container.Controls.Add(tile = (SectionBase)LoadControl(tilePath));
            Container.Controls.Add(new LiteralControl("</div>"));

            if (TileSize == TileSize.Full)
                Container.Controls.Add(new LiteralControl($"<hr class='mb-3'/>"));
