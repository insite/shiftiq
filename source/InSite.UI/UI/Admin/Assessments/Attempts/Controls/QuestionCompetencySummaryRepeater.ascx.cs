...
namespace InSite.Admin.Assessments.Attempts.Controls
{
    public partial class QuestionCompetencySummaryRepeater : SearchResultsGridViewController<QAttemptFilter>
    {
        ...
        private StandardSummary.Occupation[] Occupations
        {
        ...
        private DataTable CountQuestionPerGAC(BankState bank, IEnumerable<Question> questions, IOrderedEnumerable<StandardSummary.Gac> listToBind)
        {
            ...
            {
                var item = listToBind.Where(x => x.Code == key).FirstOrDefault();
                if (item != null)
