...
namespace InSite.UI.Admin.Assessments.Attempts.Utilities
{
    internal class AssessmentAuditReport
    {
        ...
        private static ReportVariant[] Reports => CurrentSessionState.Identity.Organization.Toolkits.Assessments.PerformanceReport.Reports;
        private static DomainItemWeight[] AssessmentTypeWeights => CurrentSessionState.Identity.Organization.Toolkits.Assessments.PerformanceReport.AssessmentTypeWeights;
        ...
        private XlsxWorksheet CreateSheet()
        {
            ...
            var columnCount = HeaderColumns + 3 * Reports.Length;
            ...
            for (int i = 0; i < Reports.Length; i++)
            {
                ...
                var requiredRole = Reports[i].RequiredRole;
        ...
        private ReportConfig CreateReportConfig(ReportVariant report)
        {
            ...
            var assessmentTypeWeights = AssessmentTypeWeights
        ...
        private static RoleColumn GetRoleColumn(int reportIndex)
        {
            var report = Reports[reportIndex];
            var name = string.Join("+", report.RoleWeights.Select(x => x.Name));

            if (report.RoleWeights.Length == 1)
                name += " only";
