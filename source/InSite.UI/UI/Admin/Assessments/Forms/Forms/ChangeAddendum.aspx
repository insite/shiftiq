        ...
        <script type="text/javascript">
            (function () {
                ...
                function initTables() {
                    ...
                    const acronymsDict = arrayToDict($acronymsInput.val().split(','));
                    const formulasDict = arrayToDict($formulasInput.val().split(','));
                    const figuresDict = arrayToDict($figuresInput.val().split(','));
                    ...
                    const $attachmentsRows = $attachmentsTable.find('> tbody > tr')
                        .prepend('<td><span class="start-move"><i class="fas fa-arrows-alt fa-fw"></i></span></td>')
                    ...
                    const $acronymsBody = addAddendumBody('acronyms', 'Acronyms');
                    const $formulasBody = addAddendumBody('formulas', 'Formulas');
                    const $figuresBody = addAddendumBody('figures', 'Figures');

                    $attachmentsRows.each(function () {
                        const $this = $(this);
                        const id = $this.data('id');

                        if (acronymsDict.hasOwnProperty(id))
                            $acronymsBody.append($this.detach())
                        else if (formulasDict.hasOwnProperty(id))
                            $formulasBody.append($this.detach());
                        else if (figuresDict.hasOwnProperty(id))
                            $figuresBody.append($this.detach());
                    ...
                    function addAddendumBody(type, title) {
                        const $result = $('<tbody>').attr('data-type', type).append(
                            $('<tr data-exclude>').append(
                                $('<th class="bg-secondary" colspan="5">').text(title)
                            )
                        );

                        $addendumTable.append($result);
                ...
                function onRowReceived(row) {
                    ...
                    if ($attachmentsTable.has($row).length) {
                        ...
                        $row.find('> td.cmd').empty().append('<span title="Remove from Addendum"><i data-action="remove-addendum" class="fas fa-trash-alt fa-fw"></i></span>');
                    }
                }
            })();
        </script>
