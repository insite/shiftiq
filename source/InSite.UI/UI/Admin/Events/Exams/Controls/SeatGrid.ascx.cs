...
namespace InSite.Admin.Events.Exams.Controls
{
    public partial class SeatGrid : SearchResultsGridViewController<QSeatFilter>
    {
        ...
        private void Grid_ItemDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            var editLink = (IconLink)e.Row.FindControl("EditSeat");
            editLink.NavigateUrl = ReturnUrlHelper.GetRedirectUrl($"/ui/admin/events/seats/edit?id={seat.SeatIdentifier}", "panel=seats");

            var deleteLink = (IconLink)e.Row.FindControl("DeleteSeat");
            deleteLink.NavigateUrl = ReturnUrlHelper.GetRedirectUrl($"/ui/admin/events/seats/delete?id={seat.SeatIdentifier}&event={seat.EventIdentifier}", "panel=seats");
        }
