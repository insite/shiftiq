...
namespace InSite.UI.Admin.Events.Exams.Forms
{
    public partial class Describe : AdminBasePage, IHasParentLinkParameters
    {
        ...
        private string OutlineUrl =>
            $"/ui/admin/events/exams/outline?event={EventIdentifier}&panel=content&tab={GetTabTitle(ContentEditor.GetCurrentTab())}&subtab={ContentEditor.GetCurrentSubTab()}";
        ...
        private void Save()
        {
            ...
            ServiceLocator.SendCommand(new DescribeEvent(EventIdentifier.Value, title, summary, description, materials, instructions, null));
