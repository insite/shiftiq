<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="StandardPopupSelectorWindow.ascx.cs" Inherits="InSite.Admin.Standards.Standards.Controls.StandardPopupSelectorWindow" %>
...
<insite:Modal runat="server" ID="ParentSelectorWindow" Title="Find and Select" CssClass="asset-popup-selector-window" Scrollable="true">
    <ContentTemplate>
        <div class="row-filter">
            <div>
                <insite:StandardTypeComboBox runat="server" ID="ParentSelectorStandardType" OnlyForCurrentOrganization="true" EmptyMessage="Standard Type" Width="100%" ZIndex="8501" />
            </div>
            <div>
        ...
        (function () {
            ...
            instance.open = function (data) {
                ...
                if ($filterTypeSelector == null) {
                    $filterTypeSelector = $modalBody.find('.row-filter #<%= ParentSelectorStandardType.ClientID %>');
                    $filterTypeSelector.on('change', onTypeChanged);
                }

                if (data.subtype != null && data.subtype.length > 0)
                    $filterTypeSelector.selectpicker('val', data.subtype);
                else
            ...
            function onRowClick(e) {
                ...
                if (typeof openerData.onSelected === 'function')
                    openerData.onSelected({ id: $this.data('id'), text: $this.data('text'), subtype: $this.data('subtype') });
            ...
            function bindTable(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var dataItem = data[i];
                    var $row = $('<tr>').attr('data-id', dataItem.id).data('text', String(dataItem.number) + ': ' + dataItem.title).data('subtype', dataItem.subtype).append(
                        (function () {
            ...
            function doSearchRequest() {
                ...
                var data = {
                    model: 2,
                    subtype: $filterTypeSelector.selectpicker('val'),
