<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="LikertTableRowGrid.ascx.cs" Inherits="InSite.Admin.Surveys.Questions.Controls.LikertTableRowGrid" %>
        ...
        (function () {
            ...
            function addItem(item) {
                ...
                item.$row = $('<tr>').data('item', item).append(
                    $('<td class="cell-icon"><span class="drag-control"><i tabindex="-1" title="Drag &amp; Drop Row" class="icon fas fa-fw fa-sort x20"></i></span></td>'),
                    $('<td class="cell-text">').html(sequence),
                    $('<td>').append($titleInput),
                    $('<td>').append($categoryInput),
                    $('<td class="cell-icon">').append(
                        $('<a tabindex="-1" title="Delete Row" href="javascript:void(0);"><i class="icon fas fa-fw fa-trash-alt x20"></i></a>').on('click', onRemoveItemClick)
                    )
