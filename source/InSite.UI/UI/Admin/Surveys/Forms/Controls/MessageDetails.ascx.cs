...
namespace InSite.Admin.Surveys.Forms.Controls
{
    public partial class MessageDetails : UserControl
    {
        ...
        {
            ChangeMessageButton.NavigateUrl = $"/ui/admin/surveys/forms/configure-workflow?survey={surveyFormIdentifier}";
            ...
            if (this.Parent != null)
            {
                ...
                if (parentTab != null)
                    ChangeMessageButton.NavigateUrl = $"/ui/admin/surveys/forms/configure-workflow?survey={surveyFormIdentifier}&returnpanel=messages&returntab={GetMessagePanelTabValue(surveyType, isInvitation)}";
            }
            ...
            {
                AddInvitationButton.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab=Invitation")
                    .GetRedirectUrl($"/ui/admin/messages/create?survey={surveyFormIdentifier}&invitation=1");
            }
            ...
            if (AddNotificationButton.Visible)
            {
                ...
                if (surveyType == SurveyMessageType.ResponseStarted)
                    AddNotificationButton.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab=ResponseStartedAdministrator")
                        .GetRedirectUrl($"/ui/admin/messages/create?survey={surveyFormIdentifier}&type=Notification&surveyType={surveyType}&notification=1");
                else if (surveyType == SurveyMessageType.ResponseCompleted)
                    AddNotificationButton.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab=ResponseCompletedAdministrator")
                        .GetRedirectUrl($"/ui/admin/messages/create?survey={surveyFormIdentifier}&type=Notification&surveyType={surveyType}&notification=1");
                else if (surveyType == SurveyMessageType.ResponseConfirmed)
                    AddNotificationButton.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab=ResponseCompletedRespondent")
                        .GetRedirectUrl($"/ui/admin/messages/create?survey={surveyFormIdentifier}&type=Notification&surveyType={surveyType}&notification=1");
            }
        }
        ...
        { 
            ...
            MessageLink.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab={GetMessagePanelTabValue(surveyType, isInvitation)}")
                        .GetRedirectUrl($"/ui/admin/messages/retitle?message={message.MessageIdentifier}");
            MessageSubjectLink.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab={GetMessagePanelTabValue(surveyType, isInvitation)}")
                        .GetRedirectUrl($"/ui/admin/messages/retitle?message={message.MessageIdentifier}");
            MessageSenderLink.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab={GetMessagePanelTabValue(surveyType, isInvitation)}")
                        .GetRedirectUrl($"/ui/admin/messages/change-sender?message={message.MessageIdentifier}");
            MessageRecipientsLink.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab={GetMessagePanelTabValue(surveyType, isInvitation)}")
                        .GetRedirectUrl($"/ui/admin/messages/subscribers/add?message={message.MessageIdentifier}");
            MessageContentLink.NavigateUrl = new ReturnUrl($"survey={surveyFormIdentifier}&panel=messages&Tab={GetMessagePanelTabValue(surveyType, isInvitation)}")
