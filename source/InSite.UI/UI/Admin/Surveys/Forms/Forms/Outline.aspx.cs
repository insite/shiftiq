...
using InSite.UI.Admin.Surveys.Forms.Controls;
...
namespace InSite.Admin.Surveys.Forms.Forms
{
    ...
    public partial class Outline : AdminBasePage
    {
        ...
        private void LoadData()
        {
            ...
            IssueWorkflowLink.NavigateUrl = $"/ui/admin/surveys/forms/configure-workflow?survey={SurveyID}";
...

            {
                ...
                if (survey.Form.Status == SurveyFormStatus.Opened && responseCount > 0)
                    QuestionsLock.AddMessage(AlertType.Warning, "Changing questions and answer options on an open survey may cause issues for users currently completing it. Please proceed with caution.");
        ...
        private void AddInstructionTabs(SurveyForm form, string panel, List<SurveyContentLabel> labels)
        {
            ...
            var field = (OutlineContentFieldList)container.LoadControl($"~/UI/Admin/Surveys/Forms/Controls/OutlineContentFieldList.ascx");
