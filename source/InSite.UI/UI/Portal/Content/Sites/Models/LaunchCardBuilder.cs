...
namespace InSite.UI.Portal.Sites
{
    public class LaunchCardBuilder
    {
        ...
        public static string GetProgress(string hook, Guid? course)
        {
            ...
            {
                var user = CurrentSessionState.Identity.User.UserIdentifier;
                var achievement = CourseSearch.BindCourseFirst(x => x.Gradebook.AchievementIdentifier, x => x.CourseIdentifier == course);
                if (achievement == null)
                    return null;

                var certificate = ServiceLocator.AchievementSearch.GetCredential(achievement.Value, user);
                if (certificate == null)
                    return null;
        ...
        public static NavItem CreatePortalNavItem(Page page, LaunchCard portal, int columnSize, Nav nav)
        {
            ...
            var caller = SurveyCaller.CreatePortal(portal.Slug).Serialize();
