...
{
    public partial class GoTo : ResponseSessionControl
    {
        ...
        protected ResponseSessionHelper.GoToInfo GoToInfo
        {
            get => (ResponseSessionHelper.GoToInfo)ViewState[nameof(GoToInfo)];
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (Current.PageNumber <= 0 || Current.PageNumber > Current.PageCount)
            {
                ErrorAlert.AddMessage(
                    AlertType.Error,
                    "fas fa-bomb",
                    $"<strong>{Translate("Invalid URL")}:</strong> {Translate("A valid survey page number is required")}.");
                return;
            }
        ...
        private void CalculateGoToPageNumber()
        {
            ...
            if (GoToType == GoToType.Previous)
                GoToInfo = ResponseSessionHelper.GoToPreviousPage(Current);

            // By default, the next page is simply the next number in the sequence. Otherwise, if the user selected
            // an option with a branch then that determines the next page.

            else
                GoToInfo = ResponseSessionHelper.GoToNextPage(Current);
        }


        private void Continue()
        {
            var goTo = GoToInfo ?? new ResponseSessionHelper.GoToInfo(1);
