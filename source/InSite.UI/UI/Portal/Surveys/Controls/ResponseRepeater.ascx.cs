...
{
    public partial class ResponseRepeater : BaseUserControl
    {
        ...
        protected RespondPage Bootstrap5Page => Page as RespondPage;
        ...
        private void SessionRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            if (dataItem.ResponseSessionCompleted.HasValue && dataItem.ResponseIsLocked)
                BindStartButton("Review", "far fa-search", ButtonStyle.Info, ResponseSessionNavigator.GetReviewPageUrl);
            else if (dataItem.ResponseSessionStarted.HasValue)
                BindStartButton("Resume", "far fa-play", ButtonStyle.Primary, ResponseSessionNavigator.GetResumePageUrl);
            else
                BindStartButton("Start", "far fa-rocket", ButtonStyle.Success, ResponseSessionNavigator.GetStartPageUrl);
            ...
            deleteButton.NavigateUrl = ResponseSessionNavigator.GetDeletePageUrl(dataItem.ResponseSessionIdentifier).ToString();
        ...
        {
            // Delete the existing response session
            ...
            // Start the response.
            ResponseSessionNavigator.RedirectTo(ResponseVerb.Start, session);
        }
