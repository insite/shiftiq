...
namespace InSite.UI.Portal.Security.Impersonation
{
    public partial class Start : PortalBasePage
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!isStart)
                Response.Redirect(Urls.StopImpersonation);

            StopButton.NavigateUrl = Urls.StopImpersonation;

            var returnUrl = StartImpersonation(UserIdentifier.Value);
        ...
        private string StartImpersonation(Guid user)
        {
            ...
            if (!Identity.IsImpersonating)
            {
                ...
                {
                    var source = typeof(Start).FullName + "." + nameof(StartImpersonation);
