...
namespace InSite.UI.Portal.Learning.Controls
{
    ...
    {
        protected List<CertificateRepeaterItem> CertificateItems
        {
            get => (List<CertificateRepeaterItem>)ViewState[nameof(CertificateItems)];
        ...
        public void BindModelToControl(List<CertificateRepeaterItem> items)
        {
        ...
        protected string GetCertificateDownloadLink()
        {
            ...
            return dataItem.CredentialStatus == Shift.Constant.CredentialStatus.Valid.ToString() && (ServiceLocator.Partition.IsE03() || dataItem.CertificationLayoutCode.IsNotEmpty())
                ? $"<a class='btn btn-sm btn-success' href=\"/ui/portal/records/credentials/certificate?credential={dataItem.CredentialIdentifier}\"><i class='far fa-award me-2'></i> Download</a>"
                : null;
        }
