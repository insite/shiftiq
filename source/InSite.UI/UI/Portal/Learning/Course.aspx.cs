    ...
    public partial class Course : PortalBasePage
    {
        ...
        private bool IsAssessmentAutoStartRequired(Domain.Organizations.OrganizationState organization)
        {
            return ServiceLocator.Partition.IsE03()
                || organization.Identifier == OrganizationIdentifiers.COTBC
                || organization.Identifier == OrganizationIdentifiers.EHRC;
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            _glossaryHelper = new GlossaryHelper(CurrentSessionState.Identity.Language);
        ...
        private void LoadButtons(Domain.CourseObjects.ProgressModel model)
        {
            ...
            RestartCourseButton.Visible = ServiceLocator.Partition.IsE03() || (Identity.IsAdministrator && !Identity.IsImpersonating);
            ...
            UnlockModulesIcon.Attributes["class"] = AreModulesUnlocked
                ? "fas fa-fw fa-unlock-keyhole me-1 text-success"
                : "fas fa-fw fa-lock-keyhole me-1 text-danger";
        ...
        private void LoadOverviewPanel()
        {
            ...
            if (gradebook != null)
            {
                if (gradebook.Achievement != null)
                {
                    ...
                    hasCertificate = achievement?.CertificateLayoutCode != null;
                }

                var gradeitem = gradebook.Items.FirstOrDefault(x => x.Achievement.HasValue);

                if (!hasCertificate && gradeitem?.Achievement != null)
                {
            ...
            CourseAchievementsLink.Visible = hasCertificate;
            ...
            if (Page.Request.QueryString["tab"] == "discussions")
                LoadDiscussionsPanel();
            else if (Page.Request.QueryString["tab"] == "achievements")
            ...
            {
                var achievements = Progress.Course.Gradebook.Items
                    .Where(x => x.Achievement != null)
                    .Select(x => x.Achievement)
                    .Distinct()
                    .ToList();
                var items = new List<CertificateRepeaterItem>();
                bool goToAchievements = true;

                foreach (var achievement in achievements)
                {
                    var a = ServiceLocator.AchievementSearch.GetAchievement(achievement.Value);
                    if (a == null)
                        continue;
                    ...
                    var c = ServiceLocator.AchievementSearch.GetCredential(achievement.Value, User.UserIdentifier);
                    item.CredentialIdentifier = c?.CredentialIdentifier;
                    item.CredentialStatus = c?.CredentialStatus ?? "Pending";
                    item.CredentialGranted = c?.CredentialGranted;
                    item.CredentialExpirationExpected = c?.CredentialExpirationExpected;
                    item.HasBadgeImage = a.HasBadgeImage;
                    item.BadgeUrl = a.BadgeImageUrl;
                    item.AchievementIdentifier = a.AchievementIdentifier;

                    if (!item.CredentialGranted.HasValue && goToAchievements)
                        goToAchievements = false;
                ...
                if (items.Count > 0)
                {
                    if (goToAchievements && (Page.Request.QueryString["tab"] != "overview" && Page.Request.QueryString["tab"] != "discussions"))
                        LoadAchievementsPanel();

                    CertificateRepeater.BindModelToControl(items);

                    ValidAlert.Visible = items.All(x => x.CredentialStatus == "Valid");

                    ExpiredAlert.Visible = items.All(x => x.CredentialStatus == "Expired");
                }
        ...
        private void LoadCurrentPage()
        {
            ...
            // If it's an incomplete assessment or survey then hide the Next button.
        ...
        private void LoadDonePanel()
        {
            ...
            DoneButton.Text = Progress.CurrentActivity.DoneButtonText.IfNullOrEmpty(TActivity.DefaultDoneButtonText);
            DoneButtonInstructions.Text = Markdown.ToHtml(Progress.CurrentActivity.DoneButtonInstructions.IfNullOrEmpty(TActivity.DefaultDoneButtonInstructions), true);
            DoneMarkedInstructions.Text = Markdown.ToHtml(Progress.CurrentActivity.DoneMarkedInstructions.IfNullOrEmpty(TActivity.DefaultDoneMarkedInstructions), true);
        ...
        private void LoadAssessmentPanel()
        {
            ...
            void SetAssessmentLink(LinkButton link, string text, string icon, string command)
            {
                ...
                link.Text = $"<i class='far fa-fw fa-{icon} me-1'></i> {text}";
                link.Visible = true;
            }
            ...
            void SetGradeLabel(string text, string icon, string category)
            {
                ...
                GradeLabel.InnerHtml = $"<i class='far fa-fw fa-{icon} me-1'></i> {Translate(text)} ";
        ...
        private void LoadSurveyPanel()
        {
            ...
            if (!string.IsNullOrWhiteSpace(body))
                ...
                SurveyBody.InnerHtml = $"<h2>{activity.Content.Title.Text[CurrentLanguage]}</h2>Please take the survey.";
            ...
            {
                var linkText = Translate("Review Survey");
                SurveyLink.InnerHtml = $"<i class='far fa-search me-1'></i> {linkText}";
                SurveyLink.HRef = $"/ui/survey/respond/review?session={lastSession.ResponseSessionIdentifier}";
            }
            else if (model.CurrentActivityResult.IsStarted && lastSession != null)
            {
                var linkText = Translate("Resume Survey");
                SurveyLink.InnerHtml = $"<i class='far fa-play me-1'></i> {linkText}";
                SurveyLink.HRef = $"/ui/survey/respond/resume?session={lastSession.ResponseSessionIdentifier}";
            }
            else
            {
                var linkText = Translate("Start Survey");
                SurveyLink.InnerHtml = $"<i class='far fa-rocket me-1'></i> {linkText}";
                SurveyLink.HRef = $"/survey/{survey.AssetNumber}/{User.UserIdentifier}";
            }
        }
