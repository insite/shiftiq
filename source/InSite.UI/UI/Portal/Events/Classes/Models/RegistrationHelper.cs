...
namespace InSite.UI.Portal.Events.Classes.Models
{
    public static class RegistrationHelper
    {
        public static void CheckMandatorySurveyResponse(QEvent @event, Guid userId)
        {
            ...
            var redirectUrl = returnUrl.GetRedirectUrl($"/survey/{surveyForm.AssetNumber}/{userId}");
        ...
        public static void SendRegistratrionCompleteAlert(Guid registrationIdentifier, Guid registeredUserIdentifier)
        {
            ...
            try
            {
                ...
                var to = userId != registeredUserIdentifier
                    ? new Guid[] { userId, registeredUserIdentifier }
                    : new Guid[] { userId };

                ServiceLocator.AlertMailer.Send(orgId, userId, alert, to, attachments);
            }
            catch (MessageNotFoundException)
            {
            }
