...
namespace InSite.UI.Portal.Accounts.Users.Controls
{
    public partial class MFAText : BaseUserControl
    {
        ...
        private bool SendMessage()
        {
            ...
            try
            {
                var response = new SmsSender(u.Value, t.Value, new ApiRequestLogger(AppSentry.SentryError))
                    .Send(CellNumber.Text, message);

                if (string.Equals(response, SmsSender.OkResponse, StringComparison.OrdinalIgnoreCase))
                    return true;
            }
            catch (Exception ex)
            {
