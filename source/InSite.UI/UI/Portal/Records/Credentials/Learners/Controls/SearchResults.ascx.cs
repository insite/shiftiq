        ...
        private string GetStatusHtml(CredentialStatus status, bool isSelfDeclared)
        {
            ...
            switch (status)
            {
                case CredentialStatus.Valid:
                    html = $"<span class='text-success'><i class='fas fa-flag-checkered fa-fw me-2'></i></span>{Translate("Valid")}";
                    break;
                case CredentialStatus.Pending:
                    html = $"<span class='text-warning'><i class='fas fa-hourglass fa-fw me-2'></i></span>{Translate("Pending")}";
                    break;
                case CredentialStatus.Expired:
                    html = $"<span class='text-danger'><i class='fas fa-brake-warning fa-fw me-2'></i></span>{Translate("Expired")}";
                    break;
            }
