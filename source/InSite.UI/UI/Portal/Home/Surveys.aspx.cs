...
using navigator = InSite.Portal.Surveys.Responses.ResponseSessionNavigator;
using uxButton = InSite.Common.Web.UI.Button;

namespace InSite.UI.Portal.Home
{
    public partial class Surveys : PortalBasePage
    {
        ...
        private void BindModelToControls(Guid user)
        {
            ...
            if (responses.Length == 0)
                StatusAlert.AddMessage(AlertType.Information, GetDisplayText("There are no Surveys to display related to your learner profile."));
        }
        ...
        private void RestartButton_Click(object sender, EventArgs e)
        {
            ...
            var commands = InSite.Portal.Surveys.Responses.Launch.BuildCommandScript("Restarted", session, survey, CurrentSessionState.Identity.User.UserIdentifier);
            foreach (var command in commands)
                ServiceLocator.SendCommand(command);

            // Start the response.
            navigator.RedirectTo(ResponseVerb.Start, session);
        }
    }
