...
";
        ...
        protected string GetExpiryHtml(object o)
        {
            ...
            return $"<span class='badge bg-{indicator}'><i class='fas fa-fw fa-{icon} me-1'></i>{verb} {TimeZones.FormatDateOnly(item.CredentialExpirationExpected.Value, User.TimeZone)}</span>";
        }
        ...
        protected string GetFlagHtml(object o)
        {
            ...
            return $"<i class='fas fa-fw fa-{icon} text-{indicator} me-1'></i>";
        }
        ...
        protected string GetStatusHtml(object o)
        {
            ...
            {
                case "Valid":
                    html.AppendLine($"<span class='text-success'><i class='fas fa-fw fa-badge-check me-1'></i>{GetDisplayText("Valid")}</span>");
                    break;
                case "Pending":
                    html.AppendLine($"<span class='text-warning'><i class='fas fa-fw fa-hourglass-start me-1'></i>{GetDisplayText("Pending")}</span>");
                    break;
                case "Expired":
                    html.Append($"<span class='text-danger'><i class='fas fa-fw fa-alarm-clock me-1'></i>{GetDisplayText("Expired")}</span>");
                    break;
            }
