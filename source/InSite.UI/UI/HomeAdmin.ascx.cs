...
namespace InSite.UI
{
    public partial class HomeAdmin : BaseUserControl
    {
        ...
        private void WarningCheck()
        {
            ...
            WarningToast.Title = "Mandatory Closed Survey";
        ...
        private string GetMandatoryClosedSurveys()
        {
            ...
            foreach (var group in groups)
            {
                ...
                if (survey.SurveyFormClosed < DateTimeOffset.UtcNow)
                    sb.Append($"The survey <a href='/ui/admin/surveys/forms/outline?survey={survey.SurveyFormIdentifier}'>{survey.SurveyFormName}</a> is now closed, but it is mandatory for the group <a href='/ui/admin/contacts/groups/edit?contact={group.GroupIdentifier}'>{group.GroupName}</a>. Group members will be unable to submit responses to it when they sign in.");
            }
