...
namespace InSite.UI.Layout.Admin
{
    public partial class AdminHome : MasterPage
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            HelpRefreshButton.Click += (x, y) => HelpTopics.Refresh();
        }
        ...
        protected void BindPlatformHelp(TAction action, HtmlGenericControl heading, HtmlGenericControl body, HyperLink edit)
        {
            ...
            if (Page is PortalBasePage p)
            {
                if (p.HelpTopic != null)
                {
                    heading.InnerHtml = p.HelpTopic.Title;
                    body.InnerHtml = Markdown.ToHtml(p.HelpTopic.GetBodyText());
                }
            }
        ...
        private void BindHelpLink()
        {
            ...
            if (Page is PortalBasePage p)
            {
                if (p?.ActionModel?.HelpUrl != null)
                {
                    ...
                    HelpColumnPlatformTitle2.InnerText = p.HelpTopic?.Title ?? p.ActionModel.ActionName;
                    ...
                    var summary = p.HelpTopic?.GetSummaryText();
                    if (!string.IsNullOrEmpty(summary))
                        HelpColumnPlatformBody2.InnerHtml = Markdown.ToHtml(summary);
                }
