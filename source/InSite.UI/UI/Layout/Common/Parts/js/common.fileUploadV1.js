(function () {
    var instance = inSite.common.fileUploadV1 = inSite.common.fileUploadV1 || {};
    ...
    function saveFile(settings) {
        ...
        let url = "/api/assets/files/legacyupload?container=" + settings.ContainerType
            + "&id=" + settings.ContainerIdentifier;
        ...
        function checkFileExtension(settings, file) {
            ...
            // Remove starting '\.', ending '$', and parentheses '()'
            const allowed = settings.AllowedExtensionsRegex.replace(/\\\./, '').replace(/\$/, '').replace(/\(|\)/g, '');

            alert(file.name + " has a file name extension that is not permitted here. Allowed extensions: " + allowed);
