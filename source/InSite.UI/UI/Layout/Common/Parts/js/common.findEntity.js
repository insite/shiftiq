    ...
    const output = (function () {
        ...
        helper.create = function (settings, data) {
            if (settings.output == 'list') {
                ...
                data.list.$checkAll = $('<button type="button" class="btn btn-sm btn-icon btn-default" data-action="check-all"><i class="far fa-fw"></i></button>');
                data.list.$clear = $('<button type="button" class="btn btn-sm btn-icon btn-default" data-action="clear"><i class="far fa-fw fa-trash-alt"></i></button>');
                data.list.$buttons = $('<div class="fe-buttons">').append(
                    data.list.$checkAll,
                    $('<button type="button" class="btn btn-sm btn-icon btn-default" data-action="add"><i class="far fa-fw fa-plus-circle"></i></button>'),
                    data.list.$clear
                );
    ...
    const modal = (function () {
        ...
        helper.create = function (settings, data) {
            ...
            if (data.multiple && data.maxSelect == 0)
                data.modal.$checkAll = $('<input type="checkbox">').on('click', onCheckAllClick);
            ...
            data.modal.$loading = $('<div class="loading-panel"><div><div><i class="fa fa-spinner fa-pulse fa-3x fa-fw"></i></div></div></div>');
            ...
            if (settings.showFooter !== false) {
                ...
                if (!data.autoSave)
                    $modalFooter.append(
                        $('<button type="button" class="btn btn-sm btn-primary" data-action="save"><i class="fas fa-fw fa-check me-1"></i> OK</button>')
                    );

                if (settings.showClear !== false)
                    $modalFooter.append(
                        $(`<button type="button" class="btn btn-sm btn-default" data-action="clear"><i class="far fa-fw fa-undo me-1"></i> ${data.strings.clearTitle}</button>`)
                    );

                $modalFooter.append(
                    $(`<button type="button" class="btn btn-sm btn-default" data-action="close"><i class="fas fa-fw fa-ban me-1"></i> ${data.strings.cancelTitle}</button>`)
                );
            ...
            function createDefaultCriteria(name, placeholder) {
                const $inputGroup = $('<div class="input-group">').append(
                    ...
                    $(`<button class="btn btn-icon btn-outline-primary" data-action="clear" type="button" title="${data.strings.clearFilterText}"><i class="fas fa-fw fa-times"></i></button>`),
                    $(`<button class="btn btn-icon btn-outline-primary" data-action="filter" type="button" title="${data.strings.applyFilterText}"><i class="fas fa-fw fa-filter"></i></button>`)
                );
                ...
                success: function (response) {
                    if (!postData.page) {
                        if (!response.count || response.count <= 0) {
                            ...
                            data.modal.$message.text('No Items');
                            return;
                        }
