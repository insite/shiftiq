...
(function (factory) {
    ...
    (function ($) {
        ...
        var cleanText = function (options, txt, nlO) {
            var out = txt;
            ...
            if (options.keepHtml) {
                ...
                out = out.replace(tS, '');
                var bT = options.badTags;
                for (var i = 0; i < bT.length; i++) {
                    tS = new RegExp('<' + bT[i] + '.*>.*<\/' + bT[i] + '>', 'gi');
                    out = out.replace(tS, '');
                }
                var allowedTags = options.keepOnlyTags;
        ...
        if (typeof $.summernote.external !== 'object')
            $.summernote.external = {};

        $.extend(true, $.summernote.external, {
            cleanText: function (txt, options) {
                var options = $.extend(false, cleanTextOptions, options);

                return cleanText(options, txt, options.newLine);
            }
        });
                        ...
                        var button = ui.button({
                            ...
                            click: function () {
                                if ($note.summernote('createRange').toString())
                                    ...
                                    $note.summernote('code', cleanText(options.cleaner, $note.summernote('code')));
                    ...
                    'summernote.paste': function (we, e) {
                        if (options.cleaner.action == 'both' || options.cleaner.action == 'paste') {
                            ...
                            if (text) {
                                if (msie || ffox)
                                    setTimeout(function () {
                                        $note.summernote('pasteHTML', cleanText(options.cleaner, text, options.cleaner.newline));
                                    }, 1);
                                else
                                    $note.summernote('pasteHTML', cleanText(options.cleaner, text, options.cleaner.newline));
                                if ($editor.find('.note-status-output').length > 0)
