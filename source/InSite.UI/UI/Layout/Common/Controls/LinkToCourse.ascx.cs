...
namespace InSite.UI.Layout.Common.Controls
{
    public partial class LinkToCourse : BlockControl, IBlockControl
    {
        ...
        private void BindCourse(string hook, string target)
        {
            ...
            {
                var v2course = CourseSearch.BindCourseFirst(x => x, x => x.OrganizationIdentifier == Organization.Identifier && x.CourseHook == hook);

                if (v2course != null)
                {
                    var content = ServiceLocator.ContentSearch.GetBlock(v2course.CourseIdentifier);
                    ...
                    var pages = ServiceLocator.PageSearch.GetCourseWebPages(v2course.CourseIdentifier);
                    if (pages.Count > 0)
                        BlockStartUrl.HRef = pages[0].Item1;
                    else
                        BlockStartUrl.HRef = ProgressState.GetPreviewUrl(v2course.CourseIdentifier);
