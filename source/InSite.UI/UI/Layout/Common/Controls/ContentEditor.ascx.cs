...
using InSite.UI.Layout.Common.Controls.Editor;
...
namespace InSite.UI.Layout.Common.Controls
{
    public partial class ContentEditor : UserControl
    {
        ...
        {
            get => (List<PillInfo>)(ViewState[nameof(NavItems)]
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            ((LobbyBasePage)Page).Translator.ExcludeFromAutoTranslate(PillsNav);
        }
        ...
        private void Section_Redirect(object sender, RedirectUrlArgs redirectArgs)
        {
            ...
            if (paramsArgs.Parameters.Count > 0)
                url = HttpResponseHelper.BuildUrl(url, string.Join("&", paramsArgs.Parameters.AllKeys.Select(key => key + "=" + paramsArgs.Parameters[key])));
        ...
        public void Add(ContentSectionDefault id, MultilingualDictionary content)
        {
            var options = LayoutContentSection.Create(id, content);

            Add(options);
        }

        public void Add(LayoutContentSection options)
        {
