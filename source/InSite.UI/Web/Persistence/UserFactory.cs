...
namespace InSite.Web.Data
{
    public class UserFactory
    {
        ...
        public static QPerson CreatePerson(Guid organizationIdentifier, Guid? creator = null)
        {
            ...
            var autoincrementPersonCode = identity?.Organization.Toolkits.Accounts?.PersonCodeAutoincrement;

            if (autoincrementPersonCode?.Enabled == true)
            {
                var number = Sequence.Increment(organizationIdentifier, SequenceType.PersonCode, autoincrementPersonCode.StartNumber);

                person.PersonCode = autoincrementPersonCode.Format.IsNotEmpty()
                    ? autoincrementPersonCode.Format.Format(number)
                    : number.ToString();
            }
