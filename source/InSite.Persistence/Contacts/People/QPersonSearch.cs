...
namespace InSite.Persistence
{
    public class QPersonSearch : IPersonSearch
    {
        ...
        private IQueryable<QPerson> CreateQuery(QPersonFilter filter, InternalDbContext db)
        {
            ...
            if (filter.ExcludeValidatorJournalSetupIdentifier.HasValue)
            {
                ...
                query = query.Where(x =>
                    !db.QJournalSetupUsers.Any(y =>
                        ...
                        && y.JournalSetupIdentifier == filter.ExcludeValidatorJournalSetupIdentifier
                        && y.UserRole == validatorRole
                    )
                );
            }

            if (filter.IncludeLearnerJournalSetupIdentifier.HasValue)
            {
                ...
                query = query.Where(x =>
                    db.QJournalSetupUsers.Any(y =>
                        y.UserIdentifier == x.UserIdentifier
                        && y.JournalSetupIdentifier == filter.IncludeLearnerJournalSetupIdentifier
                        && y.UserRole == learnerRole
                    )
                    || db.QJournalSetupGroups.Any(y =>
                        y.JournalSetupIdentifier == filter.IncludeLearnerJournalSetupIdentifier
                        && y.GroupRole == learnerRole
                        && db.QMemberships.Any(z => z.GroupIdentifier == y.GroupIdentifier && z.UserIdentifier == x.UserIdentifier)
                    )
                );
            }
