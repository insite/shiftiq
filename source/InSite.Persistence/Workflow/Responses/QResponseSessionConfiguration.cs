...
namespace InSite.Persistence
{
    public class QResponseSessionConfiguration : EntityTypeConfiguration<QResponseSession>
    {
        ...
        public QResponseSessionConfiguration(string schema)
        {
            ...
            HasRequired(a => a.Assessor).WithMany(b => b.AssessorResponseSessions).HasForeignKey(c => c.AssessorUserIdentifier).WillCascadeOnDelete(false);
            HasRequired(a => a.Respondent).WithMany(b => b.RespondentResponseSessions).HasForeignKey(c => c.RespondentUserIdentifier).WillCascadeOnDelete(false);
