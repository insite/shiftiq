    ...
    public class AchievementSearch : IAchievementSearch
    {
        ...
        private static IQueryable<VAchievement> CreateQuery(QAchievementFilter filter, InternalDbContext db)
        {
            var query = db.VAchievements
                .AsQueryable()
                .Where(x =>
                    filter.ParentOrganizationIdentifier != null
                    ? x.OrganizationIdentifier == filter.OrganizationIdentifier || x.OrganizationIdentifier == filter.ParentOrganizationIdentifier
                    : x.OrganizationIdentifier == filter.OrganizationIdentifier);
            ...
            if (filter.AchievementLabel.HasValue())
                query = query.Where(x => x.AchievementLabel.Contains(filter.AchievementLabel));
