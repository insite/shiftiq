...
";
        ...
        private static string CreateWhereForFilter(ProfileFilter filter, string searchText)
        {
            ...
            {
                where.Append(" AND OrganizationIdentifier IN (");
                for (var i = 0; i < filter.OwnerOrganizationIdentifiers.Length; i++)
                {
                    if (i > 0)
                        where.Append(", ");
                    where.Append("@OwnerOrganizationIdentifier").Append(i);
                }
                where.Append(")");
            }
        ...
        private static List<SqlParameter> GetParametersForFilter(ProfileFilter filter, string searchText)
        {
            ...
            if (filter.OwnerOrganizationIdentifiers?.Length > 0)
                for (var i = 0; i < filter.OwnerOrganizationIdentifiers.Length; i++)
                    parameters.Add(new SqlParameter("OwnerOrganizationIdentifier" + i, filter.OwnerOrganizationIdentifiers[i]));
