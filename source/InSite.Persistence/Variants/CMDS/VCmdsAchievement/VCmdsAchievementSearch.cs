...
namespace InSite.Persistence.Plugin.CMDS
{
    public static class VCmdsAchievementSearch
    {
        ...
        private static IQueryable<VCmdsAchievement> CreateQuery(VCmdsAchievementFilter filter, InternalDbContext db)
        {
            ...
            if (filter.OwnerOrganizationIdentifiers?.Length > 0)
                query = query.Where(x => filter.OwnerOrganizationIdentifiers.Contains(x.OrganizationIdentifier));
        ...
        public static List<AchievementListGridItem> SelectOrganizationAchievements(
            Guid[] ownerOrganizationIdentifiers, string keyword, Guid? departmentId = null)
        {
            using (var db = new InternalDbContext())
            {
                ...
                if (ownerOrganizationIdentifiers != null && ownerOrganizationIdentifiers.Length > 0)
                    items = items.Where(x => ownerOrganizationIdentifiers.Contains(x.OrganizationIdentifier));
