...
SELECT *
  ...
  ORDER BY RowNumber";
        ...
        private static string CreateWhereForDepartmentFilter(DepartmentFilter filter, string searchText = null)
        {
            ...
            if (filter.DepartmentIdentifiers != null && filter.DepartmentIdentifiers.Any())
            {
                var departmentList = CsvConverter.ConvertListToCsvText(filter.DepartmentIdentifiers, true);
                where.Append($" AND department.DepartmentIdentifier IN ({departmentList})");
            }
