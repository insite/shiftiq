...
namespace InSite.Persistence
{
    public static class CourseSearch
    {
        ...
        private class CourseReadHelper : ReadHelper<TCourse>
        {
            public static readonly CourseReadHelper Instance = new CourseReadHelper();

            protected override TResult ExecuteQuery<TResult>(Func<IQueryable<TCourse>, TResult> func)
            {
                using (var context = new InternalDbContext())
                {
                    ...
                    var query = context.TCourses.AsQueryable().AsNoTracking();
        ...
        private class ModuleReadHelper : ReadHelper<TModule>
        {
            public static readonly ModuleReadHelper Instance = new ModuleReadHelper();

            protected override TResult ExecuteQuery<TResult>(Func<IQueryable<TModule>, TResult> func)
            {
                using (var context = new InternalDbContext())
                {
                    context.Configuration.ProxyCreationEnabled = false;
                    ...
                    var query = context.TModules.AsQueryable().AsNoTracking();
        ...
        private class UnitReadHelper : ReadHelper<TUnit>
        {
            public static readonly UnitReadHelper Instance = new UnitReadHelper();

            protected override TResult ExecuteQuery<TResult>(Func<IQueryable<TUnit>, TResult> func)
            {
                using (var context = new InternalDbContext())
                {
                    context.Configuration.ProxyCreationEnabled = false;
                    ...
                    var query = context.TUnits.AsQueryable().AsNoTracking();
        ...
        private class ActivityReadHelper : ReadHelper<TActivity>
        {
            public static readonly ActivityReadHelper Instance = new ActivityReadHelper();

            protected override TResult ExecuteQuery<TResult>(Func<IQueryable<TActivity>, TResult> func)
            {
                using (var context = new InternalDbContext())
                {
                    context.Configuration.ProxyCreationEnabled = false;
                    ...
                    var query = context.TActivities.AsQueryable().AsNoTracking();
        ...
        public static TActivity SelectActivity(Guid id, params Expression<Func<TActivity, object>>[] includes)
        {
            return ActivityReadHelper.Instance.SelectFirst(x => x.ActivityIdentifier == id, includes);
        }

        public static TCourse SelectCourse(Guid id, params Expression<Func<TCourse, object>>[] includes)
        {
            return CourseReadHelper.Instance.SelectFirst(x => x.CourseIdentifier == id, includes);
        }

        public static TModule SelectModule(Guid id, params Expression<Func<TModule, object>>[] includes)
        {
            return ModuleReadHelper.Instance.SelectFirst(x => x.ModuleIdentifier == id, includes);
        }

        public static TUnit SelectUnit(Guid id, params Expression<Func<TUnit, object>>[] includes)
        {
            return UnitReadHelper.Instance.SelectFirst(x => x.UnitIdentifier == id, includes);
        }

        public static TUnit[] SelectUnitsForCourse(Guid id)
        {
            using (var db = new InternalDbContext(false, false))
            {
                var units = db.TUnits.AsNoTracking()
                    .Include(x => x.Modules)
                    .Where(x => x.CourseIdentifier == id)
                    .OrderBy(x => x.UnitSequence)
                    .ToArray();

                var activities = db.TActivities.AsNoTracking()
        ...
        public static TModule[] SelectModulesForCourse(Guid id)
        {
            using (var db = new InternalDbContext())
            {
                var modules = db.TModules.AsNoTracking()
        ...
        public static TModule[] SelectModulesForUnit(Guid id)
        {
            using (var db = new InternalDbContext())
            {
                var modules = db.TModules.AsNoTracking()
        ...
        public static T[] BindCourses<T>(
            Expression<Func<TCourse, T>> binder,
            Expression<Func<TCourse, bool>> filter,
        ...
        public static T[] BindUnits<T>(
            Expression<Func<TUnit, T>> binder,
            Expression<Func<TUnit, bool>> filter,
            string modelSort = null,
            string entitySort = null) =>
            UnitReadHelper.Instance.Bind(binder, filter, modelSort, entitySort);

        public static T[] BindModules<T>(
            Expression<Func<TModule, T>> binder,
            Expression<Func<TModule, bool>> filter,
            string modelSort = null,
            string entitySort = null) =>
            ModuleReadHelper.Instance.Bind(binder, filter, modelSort, entitySort);

        public static T[] BindActivities<T>(
            Expression<Func<TActivity, T>> binder,
            Expression<Func<TActivity, bool>> filter,
            string modelSort = null,
            string entitySort = null) =>
        ...
        public static int CountActivities(Expression<Func<TActivity, bool>> filter) =>
            ActivityReadHelper.Instance.Count(filter);

        public static bool ActivityExists(Expression<Func<TActivity, bool>> filter) =>
            ActivityReadHelper.Instance.Exists(filter);

        public static bool CourseExists(Expression<Func<TCourse, bool>> filter) =>
        ...
        public static bool ModuleExists(Expression<Func<TModule, bool>> filter) =>
            ModuleReadHelper.Instance.Exists(filter);

        #endregion

        #region Select (filter)

        public static int CountCourses(TCourseFilter filter)
        {
            using (var db = new InternalDbContext())
                return TCourseFilterHelper.ApplyFilter(db.TCourses.AsQueryable().AsNoTracking(), filter).Count();
        }

        public static int CountModules(Guid course)
        {
            using (var db = new InternalDbContext())
                return db.TModules.Count(x => x.Unit.CourseIdentifier == course);
        }

        public static T[] BindCourses<T>(
            Expression<Func<TCourse, T>> binder,
            TCourseFilter filter) =>
            CourseReadHelper.Instance.Bind(
                (IQueryable<TCourse> query) => query.Select(binder),
                (IQueryable<TCourse> query) => TCourseFilterHelper.ApplyFilter(query, filter),
                filter.Paging,
                null,
                filter.OrderBy
            );
        ...
        public static List<TCourse> SelectRecentCourses(Guid organizationIdentifier, int count)
        {
            using (var db = new InternalDbContext())
            {
                return db.TCourses
        ...
        public static List<TCourseUser> SelectCourseUsersForMessages()
        {
            using (var db = new InternalDbContext())
            {
                return db.TCourseUsers
                    ...
                    .Where(x => x.Course.SendMessageStalledAfterDays != null
                        ...
                        && (x.Course.CourseMessageStalledToAdministrator != null || x.Course.CourseMessageStalledToLearner != null)
        ...
        private static Course LoadCourse(Guid courseId, IRecordSearch records)
        {
            ...
            course.Framework = item.FrameworkIdentifier;
            ...
            course.CourseMessageStalledToLearner = item.CourseMessageStalledToLearner;
            course.CourseMessageStalledToAdministrator = item.CourseMessageStalledToAdministrator;
            course.CourseMessageCompletedToLearner = item.CourseMessageCompletedToLearner;
            course.CourseMessageCompletedToAdministrator = item.CourseMessageCompletedToAdministrator;
        ...
        private static void LoadGradebook(TCourse item, Course course, IRecordSearch records)
        {
        ...
        {
            var list = BindUnits(x => x, x => x.CourseIdentifier == course.Identifier, nameof(TUnit.UnitSequence))
                .OrderBy(x => x.UnitSequence)
                .ToList();
        ...
        {
            var list = BindModules(x => x, x => x.UnitIdentifier == unit.Identifier, nameof(TModule.ModuleSequence));
        ...
        {
            var list = BindActivities(x => x, x => x.ModuleIdentifier == module.Identifier, nameof(TActivity.ActivitySequence));
        ...
        public static Guid? GetFirstCourseActivity(Guid course)
        {
            using (var db = new InternalDbContext())
            {
                var activity = db.TActivities.AsNoTracking()
                    .Where(x => x.Module.Unit.CourseIdentifier == course)
        ...
        public static Guid? GetFirstCourseModule(Guid course)
        {
            using (var db = new InternalDbContext())
            {
                var module = db.TModules.AsNoTracking()
        ...
        public static Guid? GetFirstCourseUnit(Guid course)
        {
            using (var db = new InternalDbContext())
            {
                var unit = db.TUnits.AsNoTracking()
                    .Where(x => x.CourseIdentifier == course)
                    .OrderBy(x => x.UnitSequence)
                    .FirstOrDefault();
