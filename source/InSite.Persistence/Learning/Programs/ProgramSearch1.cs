    ...
    public static class ProgramSearch1
    {
        ...
        public static List<TTask> GetProgramTasks(TTaskFilter filter)
        {
            using (var db = CreateContext())
            {
                ...
                {
                    if (filter.ParentOrganizationIdentifier == null)
                        query = query.Where(x => x.OrganizationIdentifier == filter.OrganizationIdentifier);
                    else
                        query = query.Where(x => x.OrganizationIdentifier == filter.OrganizationIdentifier || x.OrganizationIdentifier == filter.ParentOrganizationIdentifier);
                }
