...
namespace InSite.Persistence
{
    public static class MembershipStore
    {
        ...
        public static Guid Save(Membership membership, bool modifyFunction = false, bool modifyEffective = false, bool modifyExpiry = false)
        {
            ...
            {
                var existing = db.Memberships.SingleOrDefault(x => x.GroupIdentifier == membership.GroupIdentifier && x.UserIdentifier == membership.UserIdentifier);

                if (existing != null)
                {
                    result = existing.MembershipIdentifier;
                    UpdateExistingMembership(membership, existing, modifyFunction, modifyEffective, modifyExpiry, script, db);
                }
                else
                {
                    result = InsertNewMembership(membership, modifyFunction, modifyEffective, modifyExpiry, script, db);
                }
