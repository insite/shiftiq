                    ...
                    <tbody>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td style=""padding: 12px 12px 12px 12px; text-align: center;"" align=""center"" bgcolor=""#ffa500"" width=""100%"">
                                                <a href=""${MessageVariable.AppUrl}/{{Survey-Path}}"" style=""color: #FFFFFF; font-family: verdana,sans-serif; font-size: 16px; font-weight: bold; text-decoration: none;"">Begin Survey</a>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                        ...
                        <tr>
                            <td style=""color: #ffa500; font-family: verdana,sans-serif; font-size: 12px; line-height: 22px;"" align=""left"" width=""100%"">
                                <a style=""color: #ffa500;"" href=""${MessageVariable.AppUrl}/{{Survey-Path}}"">${MessageVariable.AppUrl}/{{Survey-Path}}</a>
                            </td>
                        </tr>
                    </tbody>
                </table>
                            ...
                            <td style=""font-family: verdana,sans-serif; font-size: 28px; font-weight: bold; line-height: 34px;"" align=""center"" width=""100%"">
                                <table style=""border-collapse: collapse;"" align=""center"" border=""0"" cellpadding=""0"" cellspacing=""0"" width=""160"">
                                    <tbody>
                                        <tr>
                                            <td style=""padding: 12px 12px 12px 12px; text-align: center;"" align=""center"" bgcolor=""#ffa500"" width=""100%"">
                                                <a href=""${MessageVariable.AppUrl}/{{Survey-Path}}"" style=""color: #FFFFFF; font-family: verdana,sans-serif; font-size: 16px; font-weight: bold; text-decoration: none;"">Commencer le sondage</a>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
...
"
        ...
        public static string ReplacePlaceholdersForMailgun(Guid organizationId, Guid senderId, int? surveyNumber, string body, EmailVariables envelope)
        {
            ...
            ReplaceBodyPlaceholder("$Recipient-Email", envelope.RecipientEmail);
            ReplaceBodyPlaceholder("{RecipientEmail}", envelope.RecipientEmail);
            ReplaceBodyPlaceholder("{Recipient-Email}", envelope.RecipientEmail);
            ReplaceBodyPlaceholder("##Email##", envelope.RecipientEmail);
            ReplaceBodyPlaceholder("##Recipient-Email##", envelope.RecipientEmail);

            ReplaceBodyPlaceholder("{RecipientIdentifier}", envelope.RecipientIdentifier.ToString());
            ReplaceBodyPlaceholder("{Recipient-Identifier}", envelope.RecipientIdentifier.ToString());
            ReplaceBodyPlaceholder("{Recipient-Thumbprint}", envelope.RecipientIdentifier.ToString());
            ReplaceBodyPlaceholder("##RecipientIdentifier##", envelope.RecipientIdentifier.ToString());
            ReplaceBodyPlaceholder("##Recipient-Identifier##", envelope.RecipientIdentifier.ToString());
            ReplaceBodyPlaceholder("##Recipient-Thumbprint##", envelope.RecipientIdentifier.ToString());

            ReplaceBodyPlaceholder("$Recipient-First-Name", envelope.RecipientNameFirst);
            ReplaceBodyPlaceholder("{Recipient-First-Name}", envelope.RecipientNameFirst);
            ReplaceBodyPlaceholder("##FirstName##", envelope.RecipientNameFirst);

            ReplaceBodyPlaceholder("$Recipient-Last-Name", envelope.RecipientNameLast);
            ReplaceBodyPlaceholder("{Recipient-Last-Name}", envelope.RecipientNameLast);
            ReplaceBodyPlaceholder("##LastName##", envelope.RecipientNameLast);

            ReplaceBodyPlaceholder("$Recipient-Person-Code", envelope.RecipientCode);
            ReplaceBodyPlaceholder("{Recipient-Person-Code}", envelope.RecipientCode);
            ReplaceBodyPlaceholder("##PersonCode##", envelope.RecipientCode);

            ReplaceBodyPlaceholder("{Survey-Path}", $"surveys/{surveyNumber ?? 0}/{envelope.RecipientIdentifier}");
            ReplaceBodyPlaceholder("##Survey-Path##", $"surveys/{surveyNumber ?? 0}/{envelope.RecipientIdentifier}");
        ...
        public static string ReplacePlaceholdersForSmarterMail(Guid organizationId, Guid senderId, int? surveyFormAsset, string body)
        {
            ...
            ReplaceBodyPlaceholder("$RecipientEmail", "##Email##");
            ReplaceBodyPlaceholder("$Recipient-Email", "##Email##");
            ReplaceBodyPlaceholder("$Recipient-First-Name", "##FirstName##");
            ReplaceBodyPlaceholder("$Recipient-Last-Name", "##LastName##");
            ReplaceBodyPlaceholder("$Recipient-Person-Code", "##PersonCode##");

            ReplaceBodyPlaceholder("{RecipientName}", "##Name##");
            ReplaceBodyPlaceholder("{RecipientIdentifier}", "##RecipientIdentifier##");
            ReplaceBodyPlaceholder("{Recipient-Identifier}", "##RecipientIdentifier##");
            ReplaceBodyPlaceholder("{Recipient-Thumbprint}", "##RecipientIdentifier##");
            ReplaceBodyPlaceholder("{Sender-Logo}", $"<img alt='' src='{organizationLogoUrl}' />");
            ReplaceBodyPlaceholder("{Current-Year}", "$CurrentYear");
            ReplaceBodyPlaceholder("{Tenant-CompanyName}", organization.CompanyName);

            ReplaceBodyPlaceholder("{Survey-Path}", $"surveys/{surveyFormAsset ?? 0}/##RecipientIdentifier##");

            var sender = TSenderSearch.Select(senderId);
            if (sender != null)
            {
                ReplaceBodyPlaceholder("{Sender-Email}", sender.SenderEmail);
                ReplaceBodyPlaceholder("{Sender-Name}", sender.SenderName);
            }
            ...
            body = ReplacePlaceholder(body, "tenant-mailing-address", content =>
            {
                var hasContent = false;
                ...
                content = ReplacePlaceholder(
                    content,
                    "tenant-street",
                    subContent => ProcessSubContent(
                        subContent,
                        "{Tenant-Street}",
                        organization.PlatformCustomization.TenantLocation.Street));

                content = ReplacePlaceholder(content, "tenant-city", subContent =>
                {
                    var city = organization.PlatformCustomization.TenantLocation.City;
                    ...
                    subContent = ReplacePlaceholder(
                        subContent,
                        "tenant-province",
                        subContent2 => ProcessSubContent(
                            subContent2,
                            "{Tenant-Province}",
                            organization.PlatformCustomization.TenantLocation.Province));

                    return subContent.Replace("{Tenant-City}", city);
                });
                ...
                content = ReplacePlaceholder(
                    ...
                    subContent => ProcessSubContent(
                        subContent,
                        "{Tenant-PostalCode}",
                        organization.PlatformCustomization.TenantLocation.PostalCode));

                return hasContent ? content : string.Empty;

                string ProcessSubContent(string input, string name, string value)
                {
            ...
            body = ReplacePlaceholder(body, "tenant-contacts", content =>
            {
                var hasContent = false;
                ...
                content = ReplacePlaceholder(
                    ...
                    subContent => ProcessSubContent(
                        subContent,
                        "{Tenant-Phone}",
                        organization.PlatformCustomization.TenantLocation.Phone));
                ...
                content = ReplacePlaceholder(
                    ...
                    subContent => ProcessSubContent(
                        subContent,
                        "{Tenant-Mobile}",
                        organization.PlatformCustomization.TenantLocation.Mobile));
                ...
                content = ReplacePlaceholder(
                    ...
                    subContent => ProcessSubContent(
                        subContent,
                        "{Tenant-TollFree}",
                        organization.PlatformCustomization.TenantLocation.TollFree));
                ...
                content = ReplacePlaceholder(
                    ...
                    subContent => ProcessSubContent(
                        subContent,
                        "{Tenant-Fax}",
                        organization.PlatformCustomization.TenantLocation.Fax));
                ...
                content = ReplacePlaceholder(
                    ...
                    subContent => ProcessSubContent(
                        subContent,
                        "{Tenant-Email}",
                        organization.PlatformCustomization.TenantLocation.Email));
            ...
            body = ReplacePlaceholder(body, "messages-communications-office", content1 =>
            {
                var items = TCollectionItemCache
                    .Query(new TCollectionItemFilter
                    {
                        OrganizationIdentifier = organization.OrganizationIdentifier,
                        CollectionName = CollectionName.Messages_Communications_Office
                    })
                    .OrderBy(x => x.ItemFolder)
                    .ThenBy(x => x.ItemSequence)
                    .ToList();
                ...
                var keys = items
                    ...
                    .Select(x => Guid.TryParse(x.ItemName, out var key) ? key : (Guid?)null).Where(x => x.HasValue)
                    .Select(x => x.Value).Distinct().ToArray();

                var persons = PersonCriteria
                    .Bind(
                        x => new { x.UserIdentifier, x.User.FullName, x.User.Email, x.JobTitle },
                        new PersonFilter
                        {
                            OrganizationIdentifier = organization.OrganizationIdentifier,
                            IncludeUserIdentifiers = keys
                        })
                ...
                content1 = ReplacePlaceholder(content1, "messages-communications-office-item", content2 =>
                {
                    var itemContent = string.Empty;

                    foreach (var userKey in keys)
                    {
                        var user = persons[userKey];
                        var iterationContent = content2
                            .Replace("{Messages-Communications-Office-FullName}", user.FullName)
                            .Replace("{Messages-Communications-Office-Email}", user.Email);

                        iterationContent = ReplacePlaceholder(iterationContent, "messages-communications-office-item-job", content3 =>
                        {
                            return string.IsNullOrEmpty(user.JobTitle)
                                ? string.Empty
                                : content3.Replace("{Messages-Communications-Office-JobTitle}", user.JobTitle);
                        });

                        if (!string.IsNullOrEmpty(itemContent))
                            itemContent += "<br/>";

                        itemContent += iterationContent;
                    }
