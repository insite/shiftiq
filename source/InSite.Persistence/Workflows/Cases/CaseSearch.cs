...
namespace InSite.Persistence
{
    public class CaseSearch : ICaseSearch
    {
            ...
            using (var db = CreateContext())
            {
                ...
                return query
                    ...
                    .Select(x => new ExportCase
                    {
                        ...
                        TopicPersonCode = x.Issue.TopicPersonCode,
        ...
        private IQueryable<VIssue> CreateQuery(QIssueFilter filter, InternalDbContext db)
        {
            ...
            if (filter.IssueTypes.IsNotEmpty())
                query = query.Where(x => filter.IssueTypes.Contains(x.IssueType));

            if (filter.IssueStatusIdentifiers.IsNotEmpty())
                query = query.Where(x => filter.IssueStatusIdentifiers.Contains(x.IssueStatusIdentifier.Value));
