...
namespace InSite.Persistence
{
    public class QSurveyFormConfiguration : EntityTypeConfiguration<QSurveyForm>
    {
        ...
        public QSurveyFormConfiguration(string schema)
        {
            ...
            HasOptional(a => a.CreatedByUser).WithMany(b => b.CreatedSurveys).HasForeignKey(a => a.CreatedBy).WillCascadeOnDelete(false);
            HasOptional(a => a.LastChangeUserEntity).WithMany(b => b.LastChangeSurveys).HasForeignKey(a => a.LastChangeUser).WillCascadeOnDelete(false);
        }
    }
}
