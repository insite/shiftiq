...
namespace InSite.Persistence
{
    public class StorageService : IStorageService
    {
        ...
        public void RenameFile(Guid fileIdentifier, Guid userIdentifier, string newFileName)
        {
            ...
            _fileStore.RenameFile(fileIdentifier, model.FileName);
        ...
        public FileGrantStatus GetGrantStatus(Guid userId, Guid[] userRoleIds, IEnumerable<FileClaim> fileClaims)
        {
            return Authorize(userId, userRoleIds, fileClaims);
        }
        ...
        private static FileGrantStatus Authorize(ISimplePrincipal identity, FileStorageModel model)
        {
            ...
            return Authorize(identity.UserId, identity.RoleIds, model.Claims);
        }

        private static FileGrantStatus Authorize(Guid userId, Guid[] userRoleIds, IEnumerable<FileClaim> fileClaims)
        {
            if (fileClaims == null
                || !fileClaims.Any()
                || fileClaims.Any(x => x.ObjectIdentifier == userId)
                )
            {
                return FileGrantStatus.Granted;
            }
            ...
            {
                if (claim.ObjectType == FileClaimObjectType.Group && userRoleIds.Contains(claim.ObjectIdentifier))
                    return FileGrantStatus.Granted;
            }
