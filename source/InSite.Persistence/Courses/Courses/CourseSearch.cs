...
namespace InSite.Persistence
{
    public static class CourseSearch
    {
        ...
        private class VCourseReadHelper : ReadHelper<CourseMatch>
        {
            public static readonly VCourseReadHelper Instance = new VCourseReadHelper();

            protected override TResult ExecuteQuery<TResult>(Func<IQueryable<CourseMatch>, TResult> func)
            {
                using (var context = new InternalDbContext())
                {
                    context.Configuration.ProxyCreationEnabled = false;
            ...
            public T[] Bind<T>(
                Expression<Func<CourseMatch, T>> binder,
                QCourseFilter filter)
            {
                using (var context = new InternalDbContext())
                {
                    ...
                    IQueryable<T> bind(IQueryable<CourseMatch> q) => q.Select(binder);
                    IQueryable<CourseMatch> filterQuery(IQueryable<CourseMatch> q) => QCourseFilterHelper.ApplyFilter(q, filter, context);
        ...
        public static T[] BindVCourses<T>(Expression<Func<CourseMatch, T>> binder, QCourseFilter filter) =>
