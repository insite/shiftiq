...
AND q.SurveyQuestionType = 'RadioList'";
        ...
        public QSurveyRespondent GetSurveyRespondent(Guid survey, Guid user)
        {
            using (var db = CreateContext())
                return db.QSurveyRespondents
                    .AsNoTracking()
                    .Include(x => x.SurveyForm)
                    .Include(x => x.Respondent)
                    .FirstOrDefault(x => x.SurveyFormIdentifier == survey && x.RespondentUserIdentifier == user);
        }

        public List<QSurveyRespondent> GetSurveyRespondentsBySurvey(Guid survey)
        {
            using (var db = CreateContext())
                return db.QSurveyRespondents
                    .AsNoTracking()
                    .Include(x => x.SurveyForm)
                    .Where(x => x.SurveyFormIdentifier == survey).ToList();
        }

        public List<QSurveyRespondent> GetSurveyRespondentsByUser(Guid user)
        {
            using (var db = CreateContext())
                return db.QSurveyRespondents
                    .AsNoTracking()
                    .Include(x => x.SurveyForm)
                    .Where(x => x.RespondentUserIdentifier == user).ToList();
        }
