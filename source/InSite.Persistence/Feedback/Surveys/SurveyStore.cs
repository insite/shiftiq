...
namespace InSite.Persistence
{
    public class SurveyStore : ISurveyStore
    {
        ...
        public void UpdateSurvey(SurveyRespondentsAdded e)
        {
            UpdateSurvey(e, survey =>
            {
            ...
            using (var db = CreateContext())
            {
                foreach (var respondent in e.Respondents)
                {
                    var surveyRespondent = db.QSurveyRespondents.FirstOrDefault(x => x.SurveyFormIdentifier == e.AggregateIdentifier && x.RespondentUserIdentifier == respondent);
                    if (surveyRespondent == null)
                    {
                        surveyRespondent = new QSurveyRespondent
                        {
                            SurveyFormIdentifier = e.AggregateIdentifier,
                            RespondentUserIdentifier = respondent
                        };
                        db.QSurveyRespondents.Add(surveyRespondent);
                    }
                }
                db.SaveChanges();
            }
        ...
        public void UpdateSurvey(SurveyRespondentsDeleted e)
        {
            UpdateSurvey(e, survey =>
            {
            ...
            using (var db = CreateContext())
            {
                foreach (var respondent in e.Respondents)
                {
                    var surveyRespondent = db.QSurveyRespondents
                        .FirstOrDefault(x => x.SurveyFormIdentifier == e.AggregateIdentifier
                        && x.RespondentUserIdentifier == respondent);

                    if (surveyRespondent == null)
                        return;

                    db.QSurveyRespondents.Remove(surveyRespondent);
                }
                db.SaveChanges();
            }
        }
            ...
            using (var db = CreateContext())
            {
                ...
                foreach (var question in questions)
                {
                    ...
                    foreach (var answer in questionAnswers)
                    {
                        ...
                        var group = db.QGroups.FirstOrDefault(x => x.OrganizationIdentifier == organizationId && x.GroupName == groupName);
                        if (group == null)
                            continue;
