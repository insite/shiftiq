...
";
        ...
        public void UpdateMessage(MailoutDrafted2 e)
        {
            using (var db = CreateContext())
            {
                ...
                var to = new Dictionary<Guid, MailoutRecipientEmail>();
                var cc = new Dictionary<Guid, MailoutRecipientEmail>();

                if (e.To.IsNotEmpty())
                {
                    foreach (var item in e.To)
                        if (!to.ContainsKey(item.Identifier))
                            to.Add(item.Identifier, item);
                }

                if (e.Cc.IsNotEmpty())
                {
                    foreach (var item in e.Cc)
                        if (!cc.ContainsKey(item.Identifier))
                ...
                foreach (var itemTo in to)
                {
                    ...
                    var recipient = new QRecipient
                    {
                        ...
                        UserEmail = itemTo.Value.Address,
                    ...
                    if (itemTo.Value.Variables.IsNotEmpty())
                        recipient.RecipientVariables = JsonConvert.SerializeObject(itemTo.Value.Variables);
