...
";
        ...
        private IQueryable<QRecipient> CreateQueryable(InternalDbContext db, DeliveryFilter filter)
        {
            ...
            if (filter.MailoutIdentifiers.IsNotEmpty())
                q = q.Where(x => filter.MailoutIdentifiers.Contains(x.MailoutIdentifier));

            if (filter.RecipientIdentifiers.IsNotEmpty())
                q = q.Where(x => filter.RecipientIdentifiers.Contains(x.RecipientIdentifier));
