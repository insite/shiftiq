...
namespace InSite.Persistence
{
    public class EmailOutbox : IEmailOutbox
    {
        ...
        private bool CreateMailout(EmailDraft draft)
        {
            ...
            var hasRecipients = draft.Recipients != null && draft.Recipients.Count > 0;

            if (to.IsEmpty() && hasRecipients)
            {
                foreach (var recipient in draft.Recipients)
                {
                    if (recipient.Identifier == null)
            ...
            var commandRecipients = to.Select(x => new MailoutRecipientEmail(x.Key, x.Value)).ToArray();

            if (hasRecipients)
            {
                foreach (var commandRecipient in commandRecipients)
                {
                    var draftRecipient = draft.Recipients[commandRecipient.Identifier];
                    if (draftRecipient != null)
                        commandRecipient.AddVariables(draftRecipient.Variables);
                }
            }
            ...
            _commander.Send(new DraftMailout(
                ...
                sender.SenderType,
                commandRecipients,
                draft.RecipientListCc.Select(x => new MailoutRecipientEmail(x.Key, x.Value)).ToArray(),
                draft.RecipientListBcc.Select(x => new MailoutRecipientEmail(x.Key, x.Value)).ToArray(),
