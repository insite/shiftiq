...
namespace InSite.Persistence
{
    public class PageStore : IPageStore
    {
        ...
        private readonly string _bcpvpaResetCacheUrl;

        public PageStore(Action<Exception> error, string bcpvpaResetCacheUrl)
        {
            _error = error;
            _bcpvpaResetCacheUrl = bcpvpaResetCacheUrl;
        }
        ...
        public void InsertPage(PageCreated e)
        {
            ...
            ResetCache(e.OriginOrganization);
        }
        ...
        /// </remarks>
        private void ResetCache(Guid organization)
        {
            try
            {
                if (organization != Shift.Constant.OrganizationIdentifiers.BCPVPA)
                    return;

                if (LastReset != null && (DateTimeOffset.Now - LastReset.Value).TotalMinutes < 1)
                    return;

                Shift.Common.TaskRunner.RunSync(StaticHttpClient.Client.GetAsync, _bcpvpaResetCacheUrl);
            }
            catch (Exception ex)
            {
                _error?.Invoke(ex);
