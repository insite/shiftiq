    ...
    public class JournalSearch : IJournalSearch
    {
        public LogbookEnrollmentStatus GetEnrollmentStatus(Guid journalSetupIdentifier, Guid userIdentifier)
        {
            ...
            using (var db = CreateContext())
            {
                ...
                var groupEnrolled = db.QJournalSetupGroups
                    .Where(x => x.JournalSetupIdentifier == journalSetupIdentifier && x.GroupRole == role)
        ...
        private List<UserJournalDetail> GetJournalsByGroups(InternalDbContext db, Guid organizationIdentifier, Guid userIdentifier, string language)
        {
            return db.QJournalSetupGroups
                .Where(x => x.JournalSetup.OrganizationIdentifier == organizationIdentifier && x.GroupRole == JournalSetupUserRole.Learner.ToString())
                .Join(db.QMemberships.Where(x => x.UserIdentifier == userIdentifier),
                    a => a.GroupIdentifier,
                    b => b.GroupIdentifier,
                    (a, b) => a
        ...
        private static IQueryable<QJournal> CreateJournalQuery(QJournalFilter filter, InternalDbContext db)
        {
            ...
            if (filter.ValidatorUserIdentifier.HasValue)
            {
                ...
                query = query.Where(x =>
                    x.JournalSetup.Users.Any(y => y.UserIdentifier == filter.ValidatorUserIdentifier && y.UserRole == validatorRole)
                    || x.JournalSetup.Groups.Any(y =>
                        y.GroupRole == validatorRole
                        && db.QMemberships.Any(z => z.GroupIdentifier == y.GroupIdentifier && z.UserIdentifier == filter.ValidatorUserIdentifier)
                    )
                );
            }
        ...
        private static IQueryable<QExperienceCompetency> CreateExperienceCompetencyQuery(QExperienceCompetencyFilter filter, InternalDbContext db)
        {
            ...
            if (filter.ValidatorUserIdentifier.HasValue)
            {
                var validatorRole = JournalSetupUserRole.Validator.ToString();
                query = query.Where(x =>
                    x.Experience.Journal.JournalSetup.Users.Any(y => y.UserIdentifier == filter.ValidatorUserIdentifier && y.UserRole == validatorRole)
                    || x.Experience.Journal.JournalSetup.Groups.Any(y =>
                        y.GroupRole == validatorRole
                        && db.QMemberships.Any(z => z.GroupIdentifier == y.GroupIdentifier && z.UserIdentifier == filter.ValidatorUserIdentifier)
                    )
                );
        ...
        private static IQueryable<QExperience> CreateExperienceQuery(QExperienceFilter filter, InternalDbContext db)
        {
            ...
            if (filter.ValidatorUserIdentifier.HasValue)
            {
                var validatorRole = JournalSetupUserRole.Validator.ToString();
                query = query.Where(x =>
                    x.Journal.JournalSetup.Users.Any(y => y.UserIdentifier == filter.ValidatorUserIdentifier && y.UserRole == validatorRole)
                    || x.Journal.JournalSetup.Groups.Any(y =>
                        y.GroupRole == validatorRole
                        && db.QMemberships.Any(z => z.GroupIdentifier == y.GroupIdentifier && z.UserIdentifier == filter.ValidatorUserIdentifier)
                    )
                );
        ...
        private static IQueryable<QJournalSetup> CreateJournalSetupQuery(QJournalSetupFilter filter, InternalDbContext db)
        {
            ...
            if (filter.ValidatorUserIdentifier.HasValue)
            {
                var validatorRole = JournalSetupUserRole.Validator.ToString();
                query = query.Where(x =>
                    x.Users.Any(y => y.UserIdentifier == filter.ValidatorUserIdentifier && y.UserRole == validatorRole)
                    || x.Groups.Any(y =>
                        y.GroupRole == validatorRole
                        && db.QMemberships.Any(z => z.GroupIdentifier == y.GroupIdentifier && z.UserIdentifier == filter.ValidatorUserIdentifier)
                    )
            ...
            using (var db = CreateContext())
            {
                var query = CreateJournalSetupUserQuery(filter, db)
                    .Select(x => new JournalSetupUserExtended
                    {
                        ...
                        HourSum = (decimal?)db.QExperiences.Where(y => y.JournalIdentifier == x.JournalIdentifier).Sum(y => y.ExperienceHours ?? 0) ?? 0,
        ...
        public bool ExistsJournalSetupGroup(Guid journalSetupId, Guid groupId, JournalSetupUserRole groupRole)
        {
            using (var db = CreateContext())
            {
                return db.QJournalSetupGroups
                    ...
                    .Where(x =>
                        ...
                        && x.GroupRole == groupRole.ToString()
                    )
                    .Any();
            }
        }
        ...
        private static IQueryable<QJournalSetupGroup> CreateJournalSetupGroupQuery(QJournalSetupGroupFilter filter, InternalDbContext db)
        {
            var query = db.QJournalSetupGroups
                .AsNoTracking()
                .Where(x => x.JournalSetupIdentifier == filter.JournalSetupIdentifier && x.GroupRole == filter.GroupRole.ToString());
        ...
        public bool IsLogbookValidator(Guid journalSetupId, Guid userIdentifier)
        {
            var role = JournalSetupUserRole.Validator.ToString();

            using (var db = CreateContext())
            {
                if (db.QJournalSetupUsers
                    .Where(x =>
                        x.JournalSetupIdentifier == journalSetupId
                        && x.UserIdentifier == userIdentifier
                        && x.UserRole == role
                ...
                return db.QJournalSetupGroups
                    .Where(x => x.JournalSetupIdentifier == journalSetupId && x.GroupRole == role)
                    .Join(db.QMemberships.Where(x => x.UserIdentifier == userIdentifier),
                        g => g.GroupIdentifier,
                        m => m.GroupIdentifier,
                        (g, m) => g
        ...
        public List<Guid> GetJournalSetupValidatorIds(Guid journalSetupId)
        {
            var role = JournalSetupUserRole.Validator.ToString();

            using (var db = CreateContext())
            {
                var userIds1 = db.QJournalSetupUsers
                    .Where(x =>
                        x.JournalSetupIdentifier == journalSetupId
                        && x.UserRole == role
                    )
                ...
                var userIds2 = db.QJournalSetupGroups
                    .Where(x => x.JournalSetupIdentifier == journalSetupId && x.GroupRole == role)
                    .Join(db.QMemberships,
                        g => g.GroupIdentifier,
                        m => m.GroupIdentifier,
                        (g, m) => m.UserIdentifier
                    )
                    .ToList();

                return userIds1.Concat(userIds2).Distinct().ToList();
            }
        }
