...
namespace InSite.Persistence
{
    public class AchievementSearch : IAchievementSearch
    {
        ...
        public int CountCredentialsThatAllowSelfDeclaration(Guid achievementId)
        {
            using (var db = CreateContext())
                return db.VCredentials
                    .Count(x => x.AchievementIdentifier == achievementId && x.CredentialAllowSelfDeclared);
        }

        public int CountCredentialsThatDisallowSelfDeclaration(Guid achievementId)
        {
            using (var db = CreateContext())
                return db.VCredentials
                    .Count(x => x.AchievementIdentifier == achievementId && !x.CredentialAllowSelfDeclared);
        ...
        private IQueryable<VCredential> CreateQuery(VCredentialFilter filter, InternalDbContext db)
        {
            ...
            if (filter.DistrictGroupIdentifier.HasValue)
                query = query.Where(x => db.Memberships.Any(y => y.UserIdentifier == x.UserIdentifier && y.GroupIdentifier == filter.DistrictGroupIdentifier));

            if (filter.TeamGroupIdentifier.HasValue)
                query = query.Where(x => db.Memberships.Any(y => y.UserIdentifier == x.UserIdentifier && y.GroupIdentifier == filter.TeamGroupIdentifier));
